...
def stability_across_processes(
    ...
    为什么必须跨进程: 固定同一份逐位相同的 query 向量 (embedding 这一维被完全控制掉),
    在 8 个独立进程里跑纯 dense top-31, 结果**仍然不同** (1/8 的尾部换了人)。
    Chroma 的 HNSW 是近似检索, 在一堆**向量完全相同**的重复 chunk 之间, 它返回谁
    并不由 sim 决定。

    进程内**不是**量不到, 而是**严重偏向单一状态**: 单进程连跑 60 次 (每次新建
    RAGEngine + 现场 embed) 实测 top-15 出现 2 种状态, 多数派 57/60 ≈ 95%;
    且状态切换与 query 向量切换**不同步** (向量换了十几次, 状态只换了 3 次)。
    偏向这么强, 12 次的 block 有约 0.95^12 ≈ 54% 的概率全同 —— 第一版 44 个 block
    几乎全报"零变化"由此即可解释, **不需要"进程内锁死"这种更强的假设**
    (那个假设已被上面 2 种状态的实测证伪)。

    结论不变但理由要写准: 进程内取样会**系统性低估状态分布**, 所以验稳必须跨进程。
