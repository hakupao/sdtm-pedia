...
- dense-only 口径: §4.2.2 是检索 #1, 旧 gold 把这次正确召回判成 0.0 (假失分) ——
  这是补 gold 的理由。
- 生产口径 (hybrid + structured_lookup): §4.2.2 被挤出 top-15, 故**加 §4.2.2 那一步**
  改前改后同为 0.0。它买到的是判别力 (能区分"召回了 §4.2.2"和"只召回了 ch04 的
  别的节"), 不是涨分。

**q38 现值不是 0.0 而是 0.3333** (生产口径): 同一轮后续又按 gold 完整性判定补进了
第三个 AND 成员 `ch04 §4.1.6`, 它命中, 故 1/3。上面那个 0.0 描述的是加 §4.2.2
那一步的增量, 不是本题当前分数 —— 引用时别混。

两组口径的实测数字 + 本轮全部收口结论见
sdtm-rag/evidence/checkpoints/crowding_and_gold_integrity.md §2.1
"""
import yaml
