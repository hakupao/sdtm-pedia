...
**且取样必须跨独立进程** —— 进程内循环 N 次只等于 1 个样本 (§0.1)。
...
于是: 在一个进程里循环 N 次 —— 哪怕每次新建 `RAGEngine`、每次重新 embed ——**也只等于一个样本**。
第一版探针正是这么跑的, 44 个 block 每个都在自己的进程里, 所以全部报"成分零变化";
而评审用 shell `for` 循环 (每次一个**新进程**) 一跑就复现了变化。
...
14/20 进程: ch04,SV,TS,ML,DA,PR,CO,CV,TE,BS,NV,PC,UR,DD,TU
 4/20 进程: ch04,SV,TS,ML,DA,PR,CO,CV,BS,NV,PC,UR,DD,TU,OE     <- 评审 run1
 1/20 进程: ch04,SV,TS,ML,DA,PR,CO,CV,BS,NV,PC,TR,UR,DD,TU
 1/20 进程: ch04,SV,TS,ML,DA,PR,CO,CV,BS,NV,PC,RP,UR,DD,TU
```

评审 run1 与 run2 的两个状态都在这 4 种里。**`sometimes == 0` 是探针的结构性盲区造成的, 作废。**
...
> 只等于 11 题 × 2 配置 × 2 次 = 44 个独立样本, 且每个样本的抽签结果在进程内被复制了 12 遍。
...
1. **必须跨独立进程取样。这是硬要求, 不是优化。** 进程内循环 N 次 = 1 个样本 (§0.1: 同样 20 次,
   ...
   ```
   `n_procs < 2` 会直接抛错 —— 不给"1 个进程也算验过稳"留后门。**建议 n_procs ≥ 20**。
...
6. **证据里必须写这句可复现性声明** (brief 硬要求, 已按 `true` 改写, 原样抄):
   ...
   > **20 个独立进程**各跑一遍全 140 题 (跨进程取样是必须的 —— 进程内循环 N 次只等于 1 个样本),
...
.venv/bin/python -m pytest scripts/tests/test_jitter_probe.py -v
```
→ **15 passed**。brief 给的 4 条 (统计口径 + 标识可区分性护栏) 逐字落地; 另补 3 条, 覆盖后加的
...
```
→ **895 passed, 0 failed** (本 task 之前实测基线 **882**, 本 task +13)。
