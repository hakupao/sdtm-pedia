# Task 6: study_lookup 事件通道实现 + 实测 — 收口证据

> 日期: 2026-08-26 (最终修复波) · 分支 `feat/study-workflow-events` · 计划:
...
⚠ **前提, 必须先读**: 本单元交付的是 `StudyLookup` 上的一个方法 (`resolve_events`)
与它的实测, **尚未接入任何服务路径**——`server/rag.py`(只调 `resolve()`, 卡片通道)、
`server/routing_signals.py`(只调 `strong_hit()`)、`server/main.py`、`eval/run_eval.py`
均**不调用** `resolve_events` (`grep -rn resolve_events --include='*.py'` 复核, 命中
只有定义/注释/测试三类)。接线属后续单元 (plan 本来就未排接线步骤, 不是本轮偷工)。
下面的 precision/噪声数字是在这个前提下的**纯测量**, 不是线上实况——读这份文档时先确认
这个前提, 它决定这些数字该不该让人紧张。
...
子串, 最弱, 排最后)。33 题 event gold 子串口径实测 **21/33 = 63.64%**(source-recall
口径)(剔除 3 题已知泄漏题后净额 19/30 = 63.33%, 见 §3.2), 逐题清单见 §3, 12 题未命中
按机制分三桶 (§3.1, 修复轮2 重新核验修正)。**全局 precision 仅 10.81%** (259 条返回里
只有 28 条对得上 gold, 见 §6-15) ——这是在"尚未接线"前提下的测量: Ruling P2 让 6 题
转命中的同时也让通道在大多数答不出的题上从"沉默"变成"返回一堆错的", 这个代价必须和
命中率一起引用, 且**接线前须先定 precision 门槛并复测** (§6-15 已补, I-5)。卡片侧
(study golden v2) 三遍复测 **87.50% 逐题 Δ0**, 无回归。测试 1764 → **1780** (净 +16),
0 failed 0 error。spec §6 S4 自毁条款判定**未触发**, 数字见 §6-17 (此前仅存于 gitignored
文件, 本轮补进 git, 详见 A3)。
...
断言的措辞问题 (§6-16/§2 已修)。**最终整分支审查有条件合并**, 条件是文档修改 (本轮,
零代码), 已全部处置, 见 §9c/§10, 本文档收口。
...
14. **`ev_q06` 输出质量净退步, 分数不变但真实回归** (修复轮2审查 Important-1, 已实测
    ...
    **50** 条 (触顶 cap), 那 2 条被挤出结果之外, 顶替它们的是两个**与本题无关**的
    ...
    即使那 2 条没被挤掉也不算命中)。⚠ 措辞订正 (最终修复波): `resolve_events` **目前
    未接入任何服务路径** (见 §0 前提), 不存在"现在正在 union-add 进检索"这件事——
    这里说的是**若将来**把它 union-add 进检索, 这 2 条"至少指对了正确 event/activity"
    的信息会在这次修复后从结果里消失, 是一项**接线前必须知道**的输出质量倒退, 不是
    "线上已经变差了"。根因与 §3.1 桶3 相同
...
15. **全局返回量与 precision 未随命中率一起披露** (修复轮2审查 Important-2, 独立复算
    ...
    题的 precision 数字。
    ⚠ **消费前置条件 (最终修复波 I-5, 管住"使用"而不只是"引用")**: 上面这句"必须和
    命中率一起引用"管住的是**如何描述**这个通道, 没有管住**能不能直接拿去用**——
    若哪个后续单元打算把 `resolve_events` 接进检索 (union-add), **必须先给这条通道
    单独定一个 precision 门槛并重新复测**, 不能默认"能查到就接"。理由: 当前 10.81%
    的全局 precision 里, 12 个未命中题贡献了 157 条纯噪声——若不经预筛直接接线, 这
    157 条会原样进入检索候选池, 与 S3 (spec §6, 采集范围行进卡片正文导致 87.50%→84.38%
    回归) 是**同一个教训**: 往检索里塞东西的代价必须先测, 不能假设"多总比少好"。
...
16. **Tier 1b 的"整 form 让位"避让逻辑不是无损操作, "命中不丢"不是一般性结论**
    ...
    新功能"未实现, 留作后续工作。
17. **spec §6 S4 自毁条款判定补进 git (最终修复波 A3)**: 该判定此前**只活在 gitignored
    的 `EVENTS_V1_NOTES.md` 里**, 后人无法从 git 独立验证本单元当初凭什么被允许继续做
    下去——现补录: **阈值 80%** (spec §6 S4: "若 doc/卡片侧已能答出 ≥80% ⇒ 本单元价值
    不成立, 只保留 §2.4 的缺陷修复, 其余退回")。33 题 event gold 在卡片侧/doc 侧检索
    实测三口径: **卡片侧 9.1%**、**doc 侧 21.2%**、**并集 27.3%** (均为自动化代理原始值,
    未经语义核验); 对全部命中逐条读原文做语义核验后, **真阳性并集 3/33 ≈ 9.1%**
    (即 §3.2 讨论的 `ev_q08`/`ev_q28`/`ev_q30` 三题, 全部是已知的旁支泄漏, 非本单元
    交付的能力起作用)。三种口径均远低于 80% 阈值。**判定: S4 未触发。**

## 7. 引用纪律自查

⚠ **本节 2026-08-26 最终修复波前的写法是空自查 (审查方 m-3 指出)**: 旧版只搜过
"SDTM 映射已支持"这一句话, 得 0 处——但这是一个**本来就没打算写**的短语, 命中 0 是
必然的, 对判别力没有贡献。现改为搜索有真实可能被写出、且如果写了会构成过度声称的
一组措辞:

搜索「已接线 / 已上线 / 已启用 / 已生产化 / 已投入使用」: **0 处**——`resolve_events`
确实未接入任何服务路径 (§0 已明确交代), 全文没有任何一处暗示它已经接线/上线。
搜索「SDTM 映射已支持」/「TA/TE/TV/SV 已支持」: **0 处**——TA/TE/TV/SV 映射是 spec §2
明确 out of scope, 本报告未做任何相关声明。两组检索均无命中, 且第一组是本轮真正
有意义的自查项 (因为"接线"这个话题在本轮反复出现, 是最容易在措辞上说过头的地方)。
...
## 9c. 最终修复波 (2026-08-26, 最终整分支审查后) — 文档定稿, 零代码

最终整分支审查判定**有条件合并**, 条件是 4 条文档修改 (A1-A4) + 2 条 Important (I-2/I-5)
+ 3 条 Minor, **零代码改动**。审查方原话摘录: 「两轮规则 D 三方核验的全部发现均已处置且
经控制方独立复验; 我自跑的 12 项数字逐位吻合; 生产行为的净改动只有一个标签修复 (已验证
无回归) 加三个新 catalog 池 (纯增量, 无消费方回归风险)。**把一个已验证的生产缺陷修复扣在
分支上等文档, 才是更大的损失。**」逐条处置:

- **A1 (Critical) — `resolve_events` 零生产调用方须写明**: 标题改"实现 + 实测"(不再用
  "接线"); §0 加前提段, 明确 `rag.py`/`main.py`/`routing_signals.py`/`run_eval.py`
  均不调用 `resolve_events`, 10.81% precision 是"尚未接线"前提下的纯测量; §6-14 的
  "设计定位是要 union-add 进检索"改成"若将来 union-add 进检索"。**已修复**。
- **A2 — plan/代码漂移**: `docs/superpowers/plans/2026-08-25-study-workflow-events.md`
  顶部加指针 (设计基准以本 checkpoint §2 为准, cap=50/四层优先级, 非 plan 写的
  cap=8/单层), 48 个 checkbox 全部勾选。**已修复**。
- **A3 — S4 判定补进 git**: 此前只活在 gitignored 文件里, 已补入 §6-17 (阈值 80%,
  卡片侧 9.1%/doc侧 21.2%/并集 27.3%, 语义核验后真阳性 3/33, 判定未触发)。**已修复**。
- **A4 — Chain B 收尾**: `docs/PROGRESS.md`/`.work/meta/worklog/`/`CLAUDE.md` Key
  Paths/`.work/MANIFEST.md` 按需更新, 见下方逐项记录。**已修复**。
- **I-2 — spec 未标注 S3 结果**: spec §2.3 与 §5.D 各加一行 S3 退回说明。**已修复**。
- **I-5 — precision 缺消费前置条件**: §6-15 补"接线前须先定 precision 门槛并复测",
  与 S3 同一教训。**已修复**。
- **三条 Minor**: m-2 (§0 加"source-recall 口径"标注)、m-3 (§7 从空自查换成有判别力的
  搜索项)、spec §2 In scope 2 的"双向可查"限定 (输出恒为标识符, 不返回可读名)。
  **均已修复**。

**审查方额外记的一功 (原样保留, 不改写)**: 本分支犯过两次同类错误 (死代码 guard 写成
"已设闸"、归因错误复发), **两次都被独立第三方抓出来并在文档里明写"上一版是错的"**,
没有一次悄悄改掉。§3.1 那段把"自己犯了两次同一个错"写在结论正上方, 被审查方称为
"本仓见过的最诚实的自我记录形态", 该段落**不作改写**, 保留原状作范例。

## 10. 收口状态

规则 D 三轮核验 (首轮 1C/4I/3Minor + 抽检方2Minor、第二轮 1C/3I/1Minor + 审查方补充发现、
最终整分支审查 A1-A4/I-2/I-5/三条Minor) **全部已处置**。本单元遗留且明确不在收口范围内的
两个未完成项 (均已记入 §6, 不阻塞收口, 供后续单元决策): ① 名称→`assignment:` 粒度这条
索引入口仍缺 (§3.1 桶3/§6-4); ② Tier 1b "整 form 让位"应改成"逐条相减"而非当前的整体
跳过 (§6-16); ③ `resolve_events` 尚未接入任何服务路径, 接线属后续单元 (§0)。**最终整
分支审查结论: 有条件合并, 条件已全部满足。**
