...
| 5 | 测试计数链终值 1654 passed / 0 failed | 任务书 | **复现** (全程干净树, §6-5) | §6 |

findings 汇总见 §7: **F-1 HIGH (流程) · F-2 / F-3 MED (仪器结构盲点) · F-4 ~ F-7 LOW (文本精确度)**。
⚠ 声称 4 / 5 的执行时序与 controller 并发隔离协调令的关系见 **§5-1**; 声称 5 的两端全干净补跑 (最终读数) 见 **§6-5**。
...
但**出处只能写到「跑批前验过干净树」**。
⇒ **已于 controller 发出「B 已完成 + 窗口由我保证」后立即补跑, 取到两端全干净的读数
(§6-5): `1654 passed in 38.65s`, 跑批前后 `git status` 均为完全空, 源码面经 diff 证明
== 冻结 sha `9296175`。出处已升级为「全程干净」, 结论不变。** 这条至此终结。
...
⇒ 该读数后来被 §6-5 的**两端全干净**复跑取代为主读数; 本节保留作过程记录。
...
| **5** | **完全干净 (连未跟踪都无)** | **完全干净** | **1654 passed in 38.65s**, rc=0 | ✅ **主读数, 见 §6-5** |
...
### 6-3 抽检自行取窗口的四次失败 (过程记录), 及为什么结论本就不依赖它
...
同量级) ⇒ **抽检自行守候取不到两端全干净的窗口**, 如实记录, 不假装取到了。
(后由 controller 停下变异方、划出保证窗口后取到 —— 见 §6-5。这恰好说明这类窗口
**不是靠等能等到的, 必须由排程给出**, 即 finding F-1 的建议方向。)
...
⇒ 判定 **复现**。以上三条是在**没有**干净窗口时该条仍成立的理由; 干净窗口拿到之后
(§6-5) 它们从「主要依据」降为「旁证」。

### 6-5 两端全干净的补跑 (controller 保证窗口, 本条的最终读数)

controller 停下抽检 B 并划出保证窗口后, 抽检立即补跑。**跑批前后各一次
`git status --porcelain` 均为完全空** —— 不仅无已跟踪文件改动, 连未跟踪文件都没有
(抽检报告此时已被 controller 提交入库)。

```
$ git rev-parse HEAD
651880dbdfde81be4661b60715d5eeccff64fa0d
$ git diff --stat 9296175..HEAD -- sdtm-rag/server sdtm-rag/eval sdtm-rag/scripts
                                                   ← 空输出: 源码面 == 9296175
$ date -u +%Y-%m-%dT%H:%M:%SZ ;  git status --porcelain
2026-08-18T08:56:16Z                               ← 空输出
$ ./.venv/bin/python -m pytest -p no:warnings --tb=short -rN
1654 passed in 38.65s
$ echo "rc=$?" ;  git status --porcelain ;  date -u +%Y-%m-%dT%H:%M:%SZ
rc=0                                               ← 空输出
2026-08-18T08:56:56Z
```

| 核验项 | 读数 |
|---|---|
| 跑批窗口 | 08:56:16Z → 08:56:56Z (40 秒) |
| 树状态 (前 / 后) | **完全空 / 完全空** (含未跟踪) |
| HEAD | `651880d` |
| 源码面 vs 冻结 sha `9296175` | `server/` + `eval/` + `scripts/` 三处 diff **全空** |
| 全仓 diff `9296175..HEAD` | 只有两份 evidence 文件 (本报告 + 变异方报告) ⇒ **源码一字未变** |
| 结果 | **1654 passed in 38.65s**, rc=0 |
| 逐字符直方图 | `{'.': 1654}` —— 1654 个点, 0 个 `F`/`E`/`s`/`x` |

⇒ **声称 5 的出处由「跑批前干净」升级为「全程干净」**: 这一遍从头到尾工作树无任何改动,
且所加载的源码经 diff 证明与冻结 sha `9296175` 逐字节相同 (HEAD 之后只多了两份 evidence)。
判定不变 (**复现**), 证据等级上升一档。

三遍独立跑批的读数一并列出, 供看稳定性: 39.31s / 42.98s / 38.65s, **三遍均 1654 passed
/ 0 failed**, 计数零漂移。
