...
- **Content collection**: markdown sources in `../release/v1.4/*.md` (with optional `.{lang}.md` suffix), picked up by `web/src/content.config.ts`. Catchall route `[lang]/guide/[...slug]/` emits one HTML per `slug:` frontmatter. Slug `changelog` is excluded from the catchall (owned by dedicated `[lang]/changelog.astro`).
...
1. Drop `MY_DOC.{zh,en,ja}.md` into `release/v1.4/` (or `MY_DOC.md` for lang-neutral) — adjust if the current release tag in `web/src/content.config.ts` is different
