# NotebookLM 部署教程 — SDTM 知识库发布版

> 从零开始搭建一个能查 CDISC SDTM 标准 (63 域 + 176 Req 变量 + chapters + terminology + examples) 的 Google NotebookLM.
> 读完本教程你会得到: 一个可用的 NotebookLM notebook, 能准确回答 SDTM 变量定义 / Core 属性 / codelist 值 / 跨域关联 / SUPPQUAL 机制等精确查询, 同时对不在知识库的边界问题会坦诚声明 (in-KB-only 架构天然反虚构).
> 总耗时: **30-60 分钟** (含 NotebookLM Web UI 上传 + 后台 indexing, indexing 不需要死等, 见 §4).

---

## 0. 前置要求

- [ ] **Google AI Pro 订阅** (订阅页 [ai.google.com](https://ai.google.com)): NotebookLM Pro tier 上限 = 500 notebooks × 300 sources × 500K words/source × 500 chat/day × 20 audio/day; **Free tier 上限 50 sources** 在 Restricted 分享档位下套用, 本部署 42 sources 刚好在 Free tier 边界下, 但 Pro 订阅更稳
...
- [ ] **本地 clone 本仓库**: 需要上传 `current/uploads/*.md` 下的 **42 个 source 文件** + 贴 `current/instructions.md` 到 Chat Custom mode
...
- **覆盖范围**: 63/63 domains 全覆盖 / 176/176 Req 变量 ∅ gap (A4 结构级 + P3.4.5 语义级双锚闭合) / 295 md 全入库分 42 concept bucket
...
NotebookLM **没有 System Prompt** — 唯一可 prompt engineer 的入口是 Chat 的 **Custom mode** (10K char limit). 本发布版提供 `current/instructions.md` (8,925 Unicode chars, 89.25% 占用) 已经写好 SDTM 专家 prompt + 13 behavior rules + 锚点.

### 操作

1. 打开刚建好的 notebook
2. 右上角 Chat 区域找 "**Configure**" 或齿轮图标 → 选 "**Custom**" mode (默认是 "Default" / 另有 "Learning Guide")
3. **完整复制** `current/instructions.md` 的全部内容 (不要截断)
...
2. 选 "**Upload**" (或拖拽)
3. **批量选择** `current/uploads/*.md` 共 **42 个文件** (01-42 numbered buckets, 不包含 MANIFEST.md — MANIFEST 是 source 清单文档不是 source 本身)
...
本发布版 Phase 3 P3.2 实测 42/42 indexed 0 silent fail (`dev/evidence/p3_2_upload_log.md`).
...
本发布版 smoke v4 R1 测过 17 题完整题库 (Q1-Q14 + AHP1-3), 实测 **15/17 strict PASS (88.2%)**, AHP × 3 全 PASS+ 最强. 你可以复跑验证你自己部署的 notebook 同样稳.

- **题库**: `ai_platforms/SMOKE_V4.md §2`
- **逐题答案 + verdict**: `ai_platforms/notebooklm/dev/evidence/smoke_v4_answers/*.md`
- **总结报告**: `dev/evidence/smoke_v4_results.md`
...
- AHP × 3 应全 PASS+ 最强 — NotebookLM 架构天然反虚构

---

## 7. 3 档分享切换 (本平台独有)

NotebookLM 支持同一 notebook 在 3 个分享档位间动态切换, 不需要建多个 notebook. P3.9 三档切换演练 2026-04-23 VERIFIED + 深化 (`dev/evidence/share_level_toggle_drill.md`).
...
3. 回切 Restricted 会**立即 revoke** 之前的 link (P3.9 (d) 实测 PASS, 无 caching 残留)

### 重要: Public 档 ≠ 自动广播 (P3.9 新发现)
...
| smoke v4 Q9 FAIL | **预期** in-KB-only 架构限制, safety-correct PUNT | 不要改 instructions.md 加"允许外推", 会降 AHP × 3 的反虚构优势 |
...
- 若 KB 扩展 (如加 SDTMIG 下个版本 v3.5 / 新 domain), 在 `dev/scripts/bucket_config.json` 加 bucket 43+ → 跑 `merge_sources.py` → 上传新 source 增量
- 注意: ≥ 51 sources 会触发 Free tier viewer 50-cap, 要重做 P3.9 (f) Free tier 实测
...
- 重切路径: `dev/scripts/` 写新 config, pack 80-100 bucket, 重上传
- 工时估 1-2 天
...
| 文档 | 路径 | 用途 |
|------|------|------|
| **RETROSPECTIVE (本平台)** | `ai_platforms/notebooklm/docs/RETROSPECTIVE.md` | Rule C 三段 + pivot 案例 + _template/ 补丁 |
| **跨 4 平台 Phase 5 retro** | `ai_platforms/retrospectives/PHASE5_RETROSPECTIVE.md` | v1.0 FINAL 2026-04-24 Daisy 认可 4 平台 sign-off |
| **PLAN** | `ai_platforms/notebooklm/docs/PLAN.md` | 662 行 v2.2 (架构 pivot 后完整 plan) |
| **smoke v4 R1 结果** | `dev/evidence/smoke_v4_results.md` + `smoke_v4_answers/` | 17 题逐题 verdict |
| **P3.9 三档切换 evidence** | `dev/evidence/share_level_toggle_drill.md` | v1.0 FINAL 6 子步 + Public 语义深化 |
| **v1→v2 架构 pivot 记录** | `archive/v1_3notebook_SUPERSEDED_2026-04-21/ARCHITECTURE_PIVOT_RECORD.md` | 关键教训: Writer 叙事合成伪约束 |
| **reviewer 报告** | `dev/evidence/phase5_retrospective_reviewer.md` | v0.2 post-fix 独立审 10 行动项 |
...
- [ ] (可选) smoke v4 R1 17 题 ≥ 12/17 PASS
...
*发布版 v1.0 2026-04-24. Tutorial 作 Phase 5 整体 sign-off 子 gate 产物 (Daisy 2026-04-23 实操素材 + 主 session 2026-04-24 整理). 本教程配合 `RETROSPECTIVE.md` v0.2 post-reviewer-fix 同批升 FINAL.*
