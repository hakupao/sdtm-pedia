...
> 预期产出: 1 个 Gem, 3-5 个合并文件, 特色是"全量注入 1M 上下文 + 全域对比 + 跨域模式识别".
...
├── 🟡 docs/                         方法论
│   ├── README.md                    目录说明
│   └── platform_profile.md          平台特化 (Phase 0 初稿)
├── 🔵 dev/                          过程产物 (启动后填)
└── ⚫ archive/                      历史归档 (暂无)
```
...
- 知识库覆盖: (Phase 5 回填)
- 合并文件数: ~3-5 (1M tokens 窗口内)
- 总 tokens: 预计 ~500K-700K
- A/B 矩阵 PASS 比: (Phase 5 回填)
- 发布范围: **仅个人** (Gemini Gems 目前不支持团队/公开分享)
