# ChatGPT GPTs — Upload Manifest (Phase 3 Node 3 + Node 4)

> 状态: **Ready to Upload (批 1 + 批 2, 共 9 文件)**
> 最后更新: 2026-04-21 (Phase 4 N5.1 README/order 语义段手动同步, LOW L2)
> 实测 tokens 来源: `dev/evidence/_progress.json` `phases.3_execute.nodes.N2_scripts_run.product_stats` + `N4_batch2.product_stats_batch2` (去 `~` 前缀, 规则 P3)
> 总计: 9 文件 (batch 1+2 合计 2,531,313 tokens, 9/20 GPT Builder 槽位, 11 spare)

> **Note**: 本 manifest 替代了 Node 2 脚本自动追加的旧骨架 (原仅含 token + 段数列). Node 3a 新增 cap/buffer/备注/上传步骤/carry-over 段; Node 4 batch 2 实测数字由 merge 脚本 append 入下表底部. README 与上传顺序段 N5.1 同步 batch 2 (LOW L2 carry-over 关闭).

---

## 上传顺序

批 1 (01-04) 按 score 降序 (PLAN §3.2 v1.1 打分): **01 > 02 > 03 > 04** (01/02=3.4, 03/04=3.2).
批 2 (05-09) 按 Node 4 落段顺序: **05_domain_assumptions → 06_domain_examples → 07_terminology_core_high_freq → 08_terminology_quest_and_supp → 09_terminology_core_mid_tail** (域业务规则 → 示例 → 高频 CT → 问卷/补充 → 低频 CT).
完整上传顺序: **01 → 02 → 03 → 04 → 05 → 06 → 07 → 08 → 09**.
上传后**必须**等 File Search indexing 完成 (UI 显示 "Processing..." → "Ready") 再开始 smoke. Q8 Indexing indicator 可靠性 Phase 3b 实测已闭合 (5/5 PASS, `phase3_q8_indexing_reality.md` 见 _progress.json N3b).
...
5. **Instructions**: 粘贴 `current/system_prompt.md` 全文 (≤ 8,000 chars GPT Builder UI 硬上限, N5.1 v2.1 实测 7,568 bytes wc -c, buffer 5.40%)
6. **Conversation starters**: 填入 4 条 (从 `system_prompt.md` 末尾 "Conversation Starters" 段复制)
7. **Knowledge**: 上传 9 个文件 from `current/uploads/` — **顺序 01 → 02 → 03 → 04 → 05 → 06 → 07 → 08 → 09** (批 1 score 降序, 批 2 落段业务顺序)
...
- P11 文件数 ≤ 20: ✅ 批 1+2 = 9 文件, 11 spare
- P12 源溯源: ✅ 每文件内含 `<!-- source: knowledge_base/... -->` 标记 (批 1 4 文件 78 source 段 + 批 2 5 文件 217 source 段, 9 文件合计 295 source 段)
