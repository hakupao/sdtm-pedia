...
<!-- char_count: v2.2 draft (wc -m) — v2.1 live 5681 chars + v3.4 新域变量名精确校验 bullet ~450 chars = ~6130 chars estimate / budget: ~7500 chars (GPT Builder UI 硬上限口径, Phase 4 N5.1 校准) / buffer: ~18%. v2.2 changelog: +1 bullet "v3.4 新域变量名精确校验" (GFINHERT 7 字母 / 禁 GFINHERTG / CP 三 marker 8 字母 / BE+BS 短名), smoke v4 R1 Q1 ChatGPT 写成 GFINHERTG (extra G) PARTIAL MINOR carry-over fix. **v2.2 尚未应用到 GPT Builder UI**, live 仍 v2.1 (current/system_prompt.md). 待 v2.2 reviewer (候选 `superpowers:code-reviewer` 或 `oh-my-claudecode:critic`) 独立审 + 用户 ack 后应用. v2.2 base from v2.1 5681 chars (2026-04-21 N5.1 校准后). -->
