...
(or ``09_examples_data_others.md`` for --tier others).
...
def build_document(tier: str, domains: list[str]) -> tuple[str, list[str]]:
    ...
    ``full_text`` is the complete ``09_examples_data_<tier>.md`` payload.
    ...
    tier_label = "High-Frequency Domains" if tier == "high" else "Others-Tier Domains"
    out: list[str] = []
    out.append(
        f"<!-- generated by scripts_v2/extract_examples_data.py --tier {tier} at {ts} -->"
    )
    out.append("")
    out.append(f"# 09 Examples Data — {tier_label} (Stage v2.2)")
    out.append("")
    out.append(
...
def _output_path(tier: str) -> Path:
    ...
    return OUTPUT_DIR / "09_examples_data_others.md"
