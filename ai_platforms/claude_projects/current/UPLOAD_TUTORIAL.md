# Claude Project 制作教程 — SDTM 知识库发布版

> 从零开始搭建一个能查 CDISC SDTM 标准 (63 域 + terminology + chapters) 的 Claude Project.
> 读完本教程你会得到: 一个可用的 Claude Project, 能准确回答 SDTM 变量定义 / codelist 值 / example 数据表 / 跨域关联等精确查询, 同时对不在知识库的边界问题会坦诚声明.
> 总耗时: **30-90 分钟** (其中 Claude Indexing 后台耗时 30-60 分钟, 但不需要等, 见 §4).
...
- [ ] **本地 clone 本仓库**: 需要读 `current/uploads/` 下的 19 个文件和 `current/system_prompt.md`
...
3. **完整复制** `current/system_prompt.md` 的全部内容 (不要截断)
4. 粘贴到框里
5. **保存**
...
- 长尾 302 个 codelist (296 quest + 6 giants) 归属后续 Phase 7 自建 RAG (设计文档: 本项目 `docs/DESIGN_RAG_KG.md`)
...
*更多方法论细节见 `docs/` 目录下的方法论文档 (面向开发者/复盘者).*
*本教程面向最终用户/部署者, 不涉及内部开发过程和版本号细节.*
