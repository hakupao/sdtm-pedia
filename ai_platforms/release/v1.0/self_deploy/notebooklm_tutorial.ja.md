...
> 本チュートリアルを完了すると、30〜60 分で NotebookLM notebook が立ち上がります。in-KB-only による天然のハルシネーション対策を備え、smoke v4 R1 15/17 (88%) baseline を達成しています。
...
- [ ] **Google AI Pro サブスクリプション** (サブスクリプションページ [ai.google.com](https://ai.google.com)): NotebookLM Pro tier 上限 = 500 notebooks × 300 sources × 500K words/source × 500 chat/day × 20 audio/day; **Free tier 上限 50 sources** は Restricted 共有レベルで適用されます。本デプロイは 42 sources であり Free tier の境界以下に収まっていますが、Pro サブスクリプションの方がより安定します
...
- **カバレッジ**: 63/63 domains 全カバー / 176/176 Req 変数 ∅ gap (A4 構造レベル + P3.4.5 意味レベルのダブルアンカー確認済み) / 295 md を全件収録し 42 concept bucket に分類
...
### H3 VERIFIED: Custom mode は動的に切替可能
...
本公開版の Phase 3 P3.2 実測では 42/42 indexed、silent fail は 0 件でした (`../../notebooklm/dev/evidence/p3_2_upload_log.md`)。
...
本公開版は smoke v4 R1 で 17 問の完全題庫 (Q1〜Q14 + AHP1〜3) をテスト済みです。実測 **15/17 strict PASS (88.2%)**、AHP × 3 はすべて PASS+ で最高評価です。ご自身のデプロイした notebook でも同様の安定性があるかどうか、再現テストを行うことができます。
...
- AHP × 3 はすべて PASS+ が期待されます — NotebookLM のアーキテクチャはハルシネーション対策に天然の優位性があります

---

## 7. 3 段階共有レベル切替

NotebookLM は同一の notebook 内で 3 段階の共有レベルを動的に切り替えられます。複数の notebook を作成する必要はありません。P3.9 の 3 段階切替演習は 2026-04-23 に VERIFIED + 詳細確認済みです (`../../notebooklm/dev/evidence/share_level_toggle_drill.md`)。
...
3. Restricted に戻すと、以前のリンクが**即座に revoke** されます (P3.9 (d) 実測 PASS、キャッシュの残留なし)

### 重要: Public レベル ≠ 自動広報 (P3.9 の新発見)
...
| smoke v4 Q9 FAIL | **予想どおりの動作** — in-KB-only アーキテクチャの制限による safety-correct PUNT です | instructions.md に「外部推論を許可する」を追加しないでください。AHP × 3 のハルシネーション対策の優位性が低下します |
...
- 注意: ≥ 51 sources になると Free tier viewer の 50-cap が適用されます。P3.9 (f) の Free tier 実測をやり直す必要があります
...
| **smoke v4 R1 結果** | `../../notebooklm/dev/evidence/smoke_v4_results.md` + `smoke_v4_answers/` | 17 問の問ごとの verdict |
| **P3.9 3 段階切替 evidence** | `../../notebooklm/dev/evidence/share_level_toggle_drill.md` | v1.0 FINAL 6 サブステップ + Public 意味論の詳細確認 |
...
- [ ] (任意) smoke v4 R1 17 問で ≥ 12/17 PASS した
