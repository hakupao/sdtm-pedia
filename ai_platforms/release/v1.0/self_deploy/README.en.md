...
> For users who want to deploy their own copy. All 4 platforms can be deployed independently; expect to be live in 30-60 minutes.
...
1. Enter this directory (download the release pack from the release channel, or `git clone` the repository and enter `ai_platforms/release/v1.0/`). Per-platform deploy assets are already in place under `./{claude,chatgpt,gemini,notebooklm}/` (`system_prompt.md` or `instructions.md` + `uploads/` + `tutorial.<lang>.md`).
...
After deployment, run questions D0 + D1 + D5 from `../DEMO_QUESTIONS.en.md` for a quick acceptance check (5 minutes):
