...
> 读完本教程: 30-60 分钟得到一个 NotebookLM notebook, in-KB-only 天然反虚构, smoke v4 R1 15/17 (88%) baseline.
> 信源: 项目仓库 ai_platforms/notebooklm/current/ (instructions.md + uploads/ × 42).
...
- [ ] **本地 clone 本仓库**: 需要上传 `../../notebooklm/current/uploads/*.md` 下的 **42 个 source 文件** + 贴 `../../notebooklm/current/instructions.md` 到 Chat Custom mode
...
NotebookLM **没有 System Prompt** — 唯一可 prompt engineer 的入口是 Chat 的 **Custom mode** (10K char limit). 本发布版提供 `../../notebooklm/current/instructions.md` (8,925 Unicode chars, 89.25% 占用) 已经写好 SDTM 专家 prompt + 13 behavior rules + 锚点.

### 操作

1. 打开刚建好的 notebook
2. 右上角 Chat 区域找 "**Configure**" 或齿轮图标 → 选 "**Custom**" mode (默认是 "Default" / 另有 "Learning Guide")
3. **完整复制** `../../notebooklm/current/instructions.md` 的全部内容 (不要截断)
...
2. 选 "**Upload**" (或拖拽)
3. **批量选择** `../../notebooklm/current/uploads/*.md` 共 **42 个文件** (01-42 numbered buckets, 不包含 MANIFEST.md — MANIFEST 是 source 清单文档不是 source 本身)
...
本发布版 Phase 3 P3.2 实测 42/42 indexed 0 silent fail (`../../notebooklm/dev/evidence/p3_2_upload_log.md`).
...
NotebookLM 支持同一 notebook 在 3 个分享档位间动态切换, 不需要建多个 notebook. P3.9 三档切换演练 2026-04-23 VERIFIED + 深化 (`../../notebooklm/dev/evidence/share_level_toggle_drill.md`).
...
| **smoke v4 R1 结果** | `../../notebooklm/dev/evidence/smoke_v4_results.md` + `smoke_v4_answers/` | 17 题逐题 verdict |
| **P3.9 三档切换 evidence** | `../../notebooklm/dev/evidence/share_level_toggle_drill.md` | v1.0 FINAL 6 子步 + Public 语义深化 |
