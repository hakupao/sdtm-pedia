...
1. 进入本目录 (从发布渠道下载 release pack, 或 `git clone` 本仓库后进 `ai_platforms/release/v1.0/`). 各平台部署资产已就位在 `./{claude,chatgpt,gemini,notebooklm}/` (`system_prompt.md` 或 `instructions.md` + `uploads/` + `tutorial.<lang>.md`).
2. 准备账号 + 套餐 (按 §1 表格).
