...
- [ ] **本リポジトリをローカルにクローン済み**: `../../claude_projects/current/uploads/` 配下の 19 ファイルと `../../claude_projects/current/system_prompt.md` を参照する必要があります。
...
3. `../../claude_projects/current/system_prompt.md` の内容を**完全にコピー**します (切り捨てないでください)。
4. フィールドに貼り付けます。
5. **保存**します。
...
2. `../../claude_projects/current/uploads/` ディレクトリ配下の **全 19 個の `.md` ファイル**を一括選択します。
...
順序は問いません。全件アップロードしてください。一覧は `../../claude_projects/current/README.md` §アップロード一覧 をご参照ください。
