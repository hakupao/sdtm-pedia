...
本文件说明本仓库中 SDTM 知识库的构建方式, 用于保障准确性的控制措施, 以及任何单条断言如何对照源出版物进行独立审计的程序. 以这种形式公开发布, 旨在让在受监管工作中依赖本知识库的用户 — FDA / PMDA / EMA 提交; SDTM 数据编程; 映射质量控制 — 能够建立并按需重新建立其所要求的信任水平.
...
- **独立复核由设计支持.** 用户可在数秒内定位源页, 直接对照 SDTMIG PDF 进行核验. 发现差异请通过 [GitHub issue](https://github.com/hakupao/sdtm-pedia/issues) 反馈; 审计 track 是开放的, 接受外部发现.
