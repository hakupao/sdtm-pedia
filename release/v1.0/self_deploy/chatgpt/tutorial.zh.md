...
- [ ] **ChatGPT Plus ($20/月) / Pro ($200/月，高阶套餐) / Business / Enterprise / Edu** 任一套餐。**原 Team 套餐已于 2025 年改名为 Business**。Free / Go (个人低价) 套餐无法建 Custom GPT。
- [ ] **网页访问** [chatgpt.com](https://chatgpt.com): 本教程全程 Web UI 操作
- [ ] **本地 clone 本仓库**: 需要读 `./` 下的 `system_prompt.md` (约 8.6KB) 和 `uploads/` 下 9 个 .md 文件

**关于"团队共享 vs GPT Store 公开发布"**:
- Business / Enterprise / Edu 套餐内可直接共享给成员, **免审核**
...
2. 左下角点 "**Explore GPTs**" → 右上角点 "**Create**" 按钮 (或直接访问 https://chatgpt.com/create)
...
6. **Capabilities**: 关闭 **Web Search / Canvas / Image Generation / Code Interpreter & Data Analysis**。注: **DALL-E 已于 2025/12 停用，图像生成改由 GPT Image 1.5/2 承接，并统一归入 "Image Generation" 开关**。Canvas（协作编辑）与 Code Interpreter & Data Analysis 为旧名改称/扩展后名称。Web Browsing 已改名为 Web Search。
...
2. **完整复制** `./system_prompt.md` 全文 (v2.2 LIVE, 8,582 chars) 粘贴进去
3. **不能截断**: ChatGPT UI 硬上限官方为 **8,000 chars**, 实测部分情况下接受 8,582 chars (本 v2.2 LIVE 已确认). 如果 UI 拒收, 优先删除 §Conversation Starters 段
4. **保存**

**v2.2 LIVE 关键能力** (粘贴后保留, 别改):
- GFINHERT 7 字母精确 (修复 R1 GFINHERTG extra-G 拼写漂移)
- L858R / Exon 19 科学一致性主动识别 (anti-hallucination bonus)
- 3 道反虚构锚 (变量级 / 跨域级 / deprecated 级)
...
2. 选 `./uploads/` 下**全部 9 个 .md 文件**, 顺序: `01 → 02 → 03 → 04 → 05 → 06 → 07 → 08 → 09`
...
ChatGPT GPT Builder Knowledge **20 文件硬限**, 当前 9/20 (11 文件 headroom). 单文件最大 **512 MB**。用户总存储 10 GB / 组织 100 GB（聊天、Projects、GPT Knowledge 合计）。
...
打开 [`../../DEMO_QUESTIONS.zh.md`](../../DEMO_QUESTIONS.zh.md), 逐题提问 D0-D9 (10 题 demo). **一题一新 chat** 避免上下文污染. 评分 >= 8/10 = 等同基线发布版.
...
| 首 token 慢 / 频繁限流 | Plus 套餐 RAG 限流 | 等 30-60 秒重试; 持续 → Business/Enterprise |
...
**Business / Enterprise / Edu 工作区共享**:
- 免审核, Business / Enterprise / Edu 套餐内可直接共享给成员
...
- 需 OpenAI 审核 (~1-3 工作日)
- 全网可访问, 适合开放知识库
- 注意名称 / Description 不要含敏感关键词或机密代号
...
- [ ] ChatGPT Plus / Pro / Business / Enterprise / Edu 套餐已启用
- [ ] Custom GPT 已建, 命名清晰
...
*v1.1 — 2026-05-11 — UI 术语同步至 2026 年官方规范*
