...
1. Enter this directory (download the release pack from the release channel, or `git clone` the repository and enter `release/v1.0/`). Per-platform deploy assets are already in place under `./{claude,chatgpt,gemini,notebooklm}/` (`system_prompt.md` or `instructions.md` + `uploads/` + `tutorial.<lang>.md`).
