<!-- chain: J' (落地方案 + 调度链 v0.1; PLAN.md 链 J 的下游)
  修改本文件后, 必须检查:
  → branches/jp_delivery/PLAN.md                  (上流 WHAT/WHY 文書, 整合性確認)
  → branches/jp_delivery/_progress.json           (Phase 開始前 / 完了後 双方更新)
  → branches/jp_delivery/prompts/                 (各 subagent 用提示テンプレ実体)
  → branches/jp_delivery/failures/                (失敗時の retro アーカイブ先)
  → ../../.work/MANIFEST.md          (新規ファイル登録)
-->

# branches/jp_delivery/ — 落地方案 + Agent 调度設計

> 创建: 2026-04-29
> 状态: **DRAFT v0.2** — PLAN v0.3 同期 (P0 四点セット, 01 → 02 → 03 → 04 順)
> 上流: `branches/jp_delivery/PLAN.md` v0.3 (WHAT/WHY)
...
**重要な依存関係 (改訂)**:
- `01_要件定義書.xlsx` の **目的 / 業務要件 / 制約 / 前提条件** が後段 3 文書 (01 / 02 / 03) すべての根拠章になる
...
Day 1 後半 / Day 2:  ➡️  01 writer (01 の節を引用) → reviewer → 用語監査 → 用户 ack    (直列, 1 日)
...
`05_詳細設計書.xlsx` 単独. Phase 1 終了後 02 基本設計を引用しつつ単線で進める.
...
## 4. プロンプトテンプレ (実体は `branches/jp_delivery/prompts/` 配下)
