<!-- chain: J (日本同事交付链 v0.2)
  修改本文件后, 必须检查:
  → branches/jp_delivery/_progress.json              (本目录进度真源, 待 P0 创建)
  → branches/jp_delivery/CHANGELOG.md                (本目录改訂履歴, 待 P0 创建)
  → branches/jp_delivery/glossary/term_blacklist.yml (内部 jargon 黑名单, 待 P0 创建)
  → branches/jp_delivery/templates/style_guide.xlsx  (浅色 IT+医療 配色テンプレ, 待 P0 创建)
  → ../../.work/MANIFEST.md             (顶层快速参考表新增 branches/jp_delivery/ 入口)
  → ../../.work/meta/worklog.md         (执行记录追加)
  → ../../CLAUDE.md Key Paths           (新增 1 行: branches/jp_delivery/ → iTMS 向け納品目录)
-->

# branches/jp_delivery/ — iTMS 様 納品ドキュメント整備計画

> 创建: 2026-04-29
> 改訂: 2026-04-29 v0.1 → v0.2 → **v0.3** (要件定義書 を P0 に格上げ / 工程順序 01 → 02 → 03 → 04 順で着手, ユーザー判断 2026-04-29)
> 状态: **⏸ CLOSED 冻结 (2026-06-15)** — 用户决策: 日语提交产物线收口, 以后不再更新. 冻结于 DRAFT v0.3 / Phase 1 P0 2/4 v1.0 PASS + 01 v1.1-draft (reviewer/ack 未) + 中間版 v0.5 提出済; 残 P0 2 件 + Phase 2-4 + RETROSPECTIVE 不再推进. 历史只读.
...
**優先度方針 (v0.3 改訂)**: 工程順序 (要件 → 基本 → 運用 → 試験 → 詳細 → 進捗 → 横断) に整合させるため, 01 要件定義書 を P0 に格上げした. 着手順は 01 → 02 → 03 → 04 → 05 → 06 → 07 とする. ファイル番号 (NN_) は固定 (連番).

| # | 日本語文書名 | ファイル名 | 優先度 | 着手順 | Excel シート構成 | 主要参照源 | 想定工数 |
|---|------------|-----------|--------|--------|-----------------|-----------|---------|
| 1 | **要件定義書** | `01_要件定義書.xlsx` | **P0** | **1** | 表紙 / 改訂履歴 / 目次 / 1.背景目的 / 2.業務要件 / 3.機能要件 / 4.非機能要件 / 5.制約条件 / 6.前提条件 / 承認欄 | `METHODOLOGY.md` + `00_planning/` + `meta/retrospective.md` | 2-3h |
| 2 | **基本設計書** | `02_基本設計書.xlsx` | **P0** | **2** | 表紙 / 改訂履歴 / 目次 / 1.概要 / 2.全体構成 / 3.データモデル / 4.外部 IF / 5.制約事項 / 6.参考資料 / 承認欄 | `docs/DESIGN_RAG_KG.md` + schema | 3-4h |
| 3 | **運用・保守マニュアル** | `03_運用保守マニュアル.xlsx` | **P0** | **3** | 表紙 / 改訂履歴 / 目次 / 1.システム概要 / 2.ファイル構成 / 3.日常運用手順 / 4.保守作業手順 / 5.障害対応 / 6.連絡先 / 承認欄 | `README.md` + `MANIFEST.md` 抜粋 | 2-3h |
| 4 | **テスト結果報告書** | `04_テスト結果報告書.xlsx` | **P0** | **4** | 表紙 / 改訂履歴 / 目次 / 1.試験概要 / 2.試験範囲 / 3.実施結果サマリ / 4.検出事項一覧 / 5.合格判定 / 6.エビデンス一覧 / 承認欄 | `_progress.json` + `audit_matrix.md` + `evidence/` | 2-3h |
...
├── 01_要件定義書.xlsx              ★ P0 納品物
├── 02_基本設計書.xlsx              ★ P0 納品物
├── 03_運用保守マニュアル.xlsx      ★ P0 納品物
├── 04_テスト結果報告書.xlsx        ★ P0 納品物
...
【表紙シート】
  - 文書名 (大きく中央)
  - 文書番号 (例: ITMS-SDTM-02-BASIC-DESIGN-v1.0)
  - 受信者: iTMS 株式会社 御中
  - 作成: (作成者名)
  - 作成日 / 最終更新日
  - 版数 (v1.0)
...
- 個別ファイル納品時: `YYYYMMDD_ITMS-SDTM-NN_文書名_vX.Y.xlsx`
  - 例: `20260429_ITMS-SDTM-02_基本設計書_v1.0.xlsx`
...
直列推奨 (`01` 完成後 `02` `03` `04` を並列).

- [ ] `01_要件定義書.xlsx` (着手順 1) ← writer (sonnet) → reviewer (opus, 異 subagent_type) → 用語監査 (§11) → 用户 ack
  - **本旁枝で先頭に書く意図**: 01-03 全文書の根拠章となる「目的 / 業務要件 / 制約条件」を確定し, 後段文書が 01 の節を引用できるようにする.
- [ ] `02_基本設計書.xlsx` (着手順 2) ← 01 の業務 / 機能要件を引用 → 同じ 4 工程
- [ ] `03_運用保守マニュアル.xlsx` (着手順 3) ← 01 の非機能 / 制約条件を引用 → 同上
- [ ] `04_テスト結果報告書.xlsx` (着手順 4) ← 01 の前提条件で試験範囲を画定 → 同上 (エビデンス引用必須)
...
### Phase 2 — P1 補完 (1 日, v0.3 改訂で 01 を Phase 1 に移したため 05 のみ)
- [ ] `05_詳細設計書.xlsx` ← 02 基本設計の章別詳細展開
...
- 該当 issue は `branches/jp_delivery/issues_pending_jp_review.md` に蓄積 (発生時作成)
...
|--------|---------------------|---------|
| `knowledge_base/` 内容変更 | 04 テスト結果, 06 トレーサビリティ | round 単位で手動更新 |
| `docs/DESIGN_RAG_KG.md` | 02 基本設計, 05 詳細設計 | major change 時のみ |
| `.work/06_deep_verification/_progress.json` | 04 テスト結果, 07 進捗 | 節目で更新 |
| schema 変更 | 02 基本設計, 05 詳細設計 | フリーズ単位 |
| README / CLAUDE.md | 03 運用保守 | 半年に 1 回 review |
...
python scripts/audit_terms.py 02_基本設計書.xlsx --blacklist glossary/term_blacklist.yml

# 2. mapping 適用検査 (採用語が一貫使用されているか)
python scripts/audit_terms.py 02_基本設計書.xlsx --mapping glossary/term_mapping.yml --check-consistency
```
