...
writer 産物 (`branches/jp_delivery/sources/NN_<文書名>.yml` + `branches/jp_delivery/NN_<文書名>.xlsx`) を対象に, `term_blacklist.yml` (禁止語) の残存ゼロ + `term_mapping.yml` (採用語) の一貫使用 を検証する.
...
1. writer 産物 2 件: `branches/jp_delivery/sources/NN_<文書名>.yml`, `branches/jp_delivery/NN_<文書名>.xlsx`
2. 用語規律ソース: `branches/jp_delivery/glossary/term_blacklist.yml`, `branches/jp_delivery/glossary/term_mapping.yml`, `branches/jp_delivery/99_用語集.xlsx`
3. 監査スクリプト: `branches/jp_delivery/scripts/audit_terms.py`, `branches/jp_delivery/scripts/extract_xlsx_text.py`
...
```bash
# blacklist 残存検査
python3 branches/jp_delivery/scripts/audit_terms.py branches/jp_delivery/NN_<文書名>.xlsx \
  --blacklist branches/jp_delivery/glossary/term_blacklist.yml

# mapping 一貫性検査 (採用語が同一概念で一貫使用されているか)
python3 branches/jp_delivery/scripts/audit_terms.py branches/jp_delivery/NN_<文書名>.xlsx \
  --mapping branches/jp_delivery/glossary/term_mapping.yml --check-consistency
```
...
返答全体は `branches/jp_delivery/glossary/research_reports/phase_1_NN_term_audit_YYYY-MM-DD.md` として 1 ファイル.
...
- 入力: sources/NN.yml + branches/jp_delivery/NN.xlsx
