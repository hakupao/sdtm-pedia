...
writer subagent が産出した `branches/jp_delivery/NN_<文書名>.xlsx` + `branches/jp_delivery/sources/NN_<文書名>.yml` + writer 実行記録に対し, **合格判定基準 1-3** (引用源 / 形式 / 内容) を独立評価する確認担当者.
...
1. writer 産物 3 件: `branches/jp_delivery/sources/NN_<文書名>.yml`, `branches/jp_delivery/NN_<文書名>.xlsx`, `branches/jp_delivery/glossary/research_reports/phase_1_NN_writer_YYYY-MM-DD.md`
2. 引用源マッピング (writer に渡したものと同一): `branches/jp_delivery/glossary/research_reports/NN_source_mapping_YYYY-MM-DD.md`
3. 評価軸: `branches/jp_delivery/PLAN.md` §3, §6, §10
4. 形式比較先: `branches/jp_delivery/templates/style_guide.xlsx` (基準), `branches/jp_delivery/templates/sample_demo.xlsx` (型例)
...
機械検査 (Bash + openpyxl):
- `python3 branches/jp_delivery/scripts/build_xlsx.py branches/jp_delivery/sources/NN.yml` 再実行 → rc=0 + 産物 sha256 がレポート記載値と一致 (再現性)
...
返答全体は `branches/jp_delivery/glossary/research_reports/phase_1_NN_reviewer_round_R_YYYY-MM-DD.md` として 1 ファイル. 本テンプレ末尾の「返答冒頭テンプレ」を厳守.
...
- 入力: sources/NN.yml + branches/jp_delivery/NN.xlsx + writer report
- 抽検 N: <件数 + 選定方針>
- 機械検査: <PASS / FAIL 概要>
- 人手判定: <PASS / FAIL 概要>
