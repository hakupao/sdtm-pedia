# branches/jp_delivery/ — iTMS 様 納品ドキュメント (Internal Index)

> **⏸ CLOSED 冻结 (2026-06-15)**: 本旁枝 (日语提交产物线) 已收口, 以后不再更新 (用户决策). 冻结于 Phase 1 P0 2/4 + 01 v1.1-draft + 中間版 v0.5; 残工程不再推进. 以下内容历史只读.
>
...
| `_progress.json` | 進捗真源 (Tier 2 schema) | CLOSED 冻结 |
| `CHANGELOG.md` | 改訂履歴一元管理 | CLOSED 冻结 |
| `WORKLOG.md` | 日次作業ログ (日本語, iTMS 様可視) | CLOSED 冻结 |
| `glossary/term_blacklist.yml` | 使用禁止語リスト | CLOSED 冻结 |
| `glossary/term_mapping.yml` | 用語写像辞書 | v0.5 active |
| `glossary/reference_standards.md` | 参照規格一覧 | CLOSED 冻结 |
| `templates/style_guide.xlsx` | 配色 / フォント テンプレ | Phase 0.7 PASS |
| `templates/cover_template.xlsx` | 表紙統一様式 | Phase 0.7 PASS |
| `scripts/build_xlsx.py` | yml → xlsx 生成 (schema 検証付き) | CLOSED 冻结 |
| `scripts/audit_terms.py` | 用語規律監査 | CLOSED 冻结 |

### 納品物 (.xlsx)

| ファイル | 優先度 | 着手順 | 状態 |
|---------|-------|--------|------|
| `01_要件定義書.xlsx` | **P0** | 1 | ✅ v1.0 PASS 2026-04-29 |
| `02_基本設計書.xlsx` | **P0** | 2 | ✅ v1.0 PASS 2026-04-29 |
| `03_運用保守マニュアル.xlsx` | **P0** | 3 | 未着手 |
| `04_テスト結果報告書.xlsx` | **P0** | 4 | 未着手 |
...
5. **Phase 1** P0 四点セット (01 → 02 → 03 → 04) ⏸ CLOSED 冻结 2/4 (01 + 02 PASS; 中間版 v0.5 提出済 2026-04-30; 残工程冻结不再推进)
