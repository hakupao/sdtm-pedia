...
`python3 branches/jp_delivery/scripts/build_xlsx.py branches/jp_delivery/sources/sample_demo.yml` を実行し、
...
| 対象 | 結果 |
|------|------|
| `branches/jp_delivery/scripts/build_xlsx.py` | デバッグ成果物なし (PASS) |
| `branches/jp_delivery/sources/99_用語集.yml` | `note_rows: [0]` × 3 件追加のみ (構造変更のみ) |
| `branches/jp_delivery/sources/sample_demo.yml` | 変更なし |
...
| ファイル | 変更内容 |
|---------|---------|
| `branches/jp_delivery/scripts/build_xlsx.py` | LOW-A/B 実装, 877 行 (旧 754 行 +123 行) |
| `branches/jp_delivery/sources/99_用語集.yml` | 3 シートに `note_rows: [0]` 追加 |
| `branches/jp_delivery/99_用語集.xlsx` | 再生成 (24,888 bytes) |
| `branches/jp_delivery/templates/sample_demo.xlsx` | 再生成 (13,150 bytes) |
| `branches/jp_delivery/templates/.backup/sample_demo.xlsx.before_low_patch` | バックアップ (変更前) |
| `branches/jp_delivery/templates/.backup/99_用語集.xlsx.before_low_patch` | バックアップ (変更前) |
