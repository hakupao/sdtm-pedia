...
> 上流文書: `branches/jp_delivery/PLAN.md` §3, `branches/jp_delivery/EXECUTION_PLAN.md` §3 Phase 0.7
...
- 生成ファイル: `branches/jp_delivery/templates/sample_demo.xlsx`
...
yml schema は拡張余地を残している。`sheets[].type` に `image` (assets/ 内 PNG 挿入) や `conditional_table` (条件付き書式付き表) を追加する場合は `build_xlsx.py` の `type_handlers` 辞書に対応関数を追加するだけでよく、既存シート種別への影響はない。7 文書の writer が `branches/jp_delivery/sources/NN.yml` を整備し `python3 branches/jp_delivery/scripts/build_xlsx.py branches/jp_delivery/sources/NN.yml` を実行するだけで PLAN §3 形式約定 100% 準拠の xlsx を得られる体制が整った。
