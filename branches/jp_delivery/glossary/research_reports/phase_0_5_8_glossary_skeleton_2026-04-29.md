...
| 項目 | 内容 |
|------|------|
| 入力 | `branches/jp_delivery/glossary/term_mapping.yml` v0.3 (33 entries, 542 行) |
| 補助入力 | `branches/jp_delivery/glossary/term_blacklist.yml` (33 語) |
| 出力 1 | `branches/jp_delivery/sources/99_用語集.yml` (build_xlsx.py 入力用) |
| 出力 2 | `branches/jp_delivery/99_用語集.xlsx` (7 シート, 24,841 bytes) |
| 出力 3 | 本ファイル |
| 補助スクリプト | `branches/jp_delivery/scripts/yml_to_glossary.py` (新規, 単機能変換ヘルパ) |
...
Phase 3 仕上げ時に Bojiang または後段担当者が `branches/jp_delivery/sources/99_用語集.yml` の当該セルを直接編集し, `python3 branches/jp_delivery/scripts/build_xlsx.py branches/jp_delivery/sources/99_用語集.yml` を再実行することで xlsx を更新できる. `yml_to_glossary.py` は再実行不要 (yml を直接編集する).
...
- 検査スコープ: `branches/jp_delivery/99_用語集.xlsx` 全 7 シート
- 除外適用:
