...
<div id="app">
  ...
  <main>
    <svg id="svg"><g id="vp"><g id="edges"></g><g id="nodes"></g></g></svg>
    <div id="count"></div>
    <div class="legend" id="legend"></div>
    <div class="hint">悬停看全名 · 点击节点看关系 · 拖拽 · 滚轮缩放 · 空白平移</div>
...
// ---- render ----
function draw(){
  for(const e of E){ e.el.setAttribute("x1",e.a.x);e.el.setAttribute("y1",e.a.y);
...
function build(view){
  ...
  E=view.edges.map(e=>({a:byId.get(e.s),b:byId.get(e.t),adv:e.adv})).filter(e=>e.a&&e.b);
  seed(N);
  for(const e of E){ const l=document.createElementNS(SVGNS,"line");
    l.setAttribute("class","edge"+(e.adv?" adv":"")); e.el=l; gEdges.appendChild(l); }
...
function startDrag(n,ev){ ev.stopPropagation(); drag=n; n.fix=1;
  ...
  const up=()=>{ if(drag){drag.fix=0; if(moved<4)openPanel(n); drag=null;} removeEventListener("pointermove",move); removeEventListener("pointerup",up); };
...
function syncUI(){
  ...
  $("#domSel").classList.toggle("hide",cur.v!=="domain");
  $("#codeSel").classList.toggle("hide",cur.v!=="impact");
  if(cur.dom)$("#domSel").value=cur.dom; if(cur.code)$("#codeSel").value=cur.code;
}
$("#views").addEventListener("click",ev=>{ const b=ev.target.closest("button"); if(!b)return; cur.v=b.dataset.v; render(); });
$("#domSel").addEventListener("change",e=>{cur.dom=e.target.value;render();});
...
$("#panel").addEventListener("click",ev=>{ if(ev.target.id==="pClose"){closePanel();return;}
  const el=ev.target.closest("[data-code]"); if(!el)return;
  if(el.classList.contains("navcode"))nav("impact",el.dataset.code); else nav("domain",el.dataset.code); });
