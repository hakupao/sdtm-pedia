...
源材料を読み取り, `branches/jp_delivery/sources/NN_<文書名>.yml` の `content` フィールドを充填し, `branches/jp_delivery/scripts/build_xlsx.py` で .xlsx を産出する.
...
- 文書間の一貫性: `branches/jp_delivery/templates/style_guide.xlsx` + `cover_template.xlsx` に統一済 (writer は触らない).
- 用語規律: `branches/jp_delivery/glossary/term_blacklist.yml` (禁止語) + `term_mapping.yml` (採用語) を厳守.

## 入力 (派発側が提示)

1. 文書番号 + 文書名 (例: ITMS-SDTM-01 要件定義書)
2. yml 骨格ファイルパス (`branches/jp_delivery/sources/NN_<文書名>.yml`) — 本テンプレで充填する対象
3. 引用源マッピング (例: `branches/jp_delivery/glossary/research_reports/NN_source_mapping_YYYY-MM-DD.md`)
4. 並行参照必須: `branches/jp_delivery/PLAN.md` §1 §3 §10 §11 / `branches/jp_delivery/glossary/term_blacklist.yml` / `term_mapping.yml`
5. 並行参照任意: `branches/jp_delivery/templates/sample_demo.xlsx` (yml→xlsx 動作例)

## 産物 (必須 3 件)

1. **`branches/jp_delivery/sources/NN_<文書名>.yml`** — `content` フィールド全充填済 (placeholder 残ゼロ)
2. **`branches/jp_delivery/NN_<文書名>.xlsx`** — `python3 branches/jp_delivery/scripts/build_xlsx.py branches/jp_delivery/sources/NN_<文書名>.yml` で生成
3. **`branches/jp_delivery/glossary/research_reports/phase_1_NN_writer_YYYY-MM-DD.md`** — 実行記録 (各シート content の根拠 + 自己検証結果)
...
詳細は `branches/jp_delivery/scripts/build_xlsx.py` 各 `_sheet_*` 関数の docstring 参照.
...
| 2 | yml schema 通過 | `python3 branches/jp_delivery/scripts/build_xlsx.py …` rc=0 |
...
- 上限超過 → `branches/jp_delivery/failures/phase_1_NN_writer_attempt_X.md` に規律 B (失敗アーカイブ) で記録 + 主 session に escalate.
...
- 着手: YYYY-MM-DD
- 引用源件数: <N 件 / 全 M 件>
- 産物: sources/NN.yml + branches/jp_delivery/NN.xlsx + 本レポート
