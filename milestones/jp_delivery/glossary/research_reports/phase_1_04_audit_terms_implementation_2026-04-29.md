...
| ファイル | 行数 | 役割 |
|---|---|---|
| `branches/jp_delivery/scripts/extract_xlsx_text.py` | 164 行 | .xlsx → 全文 plain text 抽出 (公開 API: `extract_cells`) |
| `branches/jp_delivery/scripts/audit_terms.py` | 538 行 | 用語規律監査 (blacklist + mapping consistency) |
...
```
コマンド: python3 branches/jp_delivery/scripts/extract_xlsx_text.py branches/jp_delivery/99_用語集.xlsx --format jsonl
```
...
コマンド: python3 branches/jp_delivery/scripts/audit_terms.py branches/jp_delivery/99_用語集.xlsx \
  ...
  --mapping branches/jp_delivery/glossary/term_mapping.yml --check-consistency --human
```
