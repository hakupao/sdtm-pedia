# doc 轨 U1 续跑交接 — 从 Task 8 (doc-only 上界基线) 起

> 更新: 2026-08-12 · 路由词: **「U1 续跑 开始任务」**
> 上一 session 完成 Task 6 (出题批 2) 与 Task 7 (出题批 3 + L1 池), **题集已达 30/30**。
> 数据红线: 本文件进 git, **零真名零正文**。题集/笔记/失败归档全在 `sdtm-rag/data/study/`(gitignored)。

## 0. 现状一句话

30 题计分池**已建成并三轮独立审过**。下一步 Task 8: 跑 doc-only 上界基线, **对三条自毁条款**。
**这是全程唯一没有裁量空间的一步** —— 阈值 2026-08-11 定稿写死, 跑出来落在哪就是哪。
