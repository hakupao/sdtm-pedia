...
| 3 | ~~**C2** (933/212 页表单 PDF)~~ → ⛔ **两份均判 DROP; Plan C 整条线关闭 (2026-08-25)** | — | C2-pre 勘察实测: 两份 PDF 相对源数据**零增量**。**根因不是 PDF 没内容, 是 ConfigReport 有 3 个 sheet 从未被解析** (`Study workflow-{Events,Activities,Forms}`; 其中 Forms 表 113 行直接给出 Event/Activity/Form 分配 + Repeating + Hidden items, **带 OID 可 join**)。**改做: 解析那 3 个 sheet (确定性 xlsx, 零有损, 不属 Plan C)**。另查到生产缺陷: 卡片 `適用範囲` 行实为 `Hidden in activity` (语义相反, 231/961 张)。证据 `sdtm-rag/evidence/checkpoints/c2_pre_survey.md` §8 |
...
- **20 (C2-pre 新增). 「增量」必须相对*源*测, 不是相对*当前产物*测** —— C2-pre 把基线定成 `catalog.json` (仅由 ConfigReport 7 个 sheet 中的 3 个建成), 于是把「管线没解析」误判成「源里没有」, 一度给出错误判决。**任何勘察的第一步是枚举源的全部 sheet / 全部区段并标出哪些已被解析**, 再定基线。
