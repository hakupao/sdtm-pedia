...
## 0′. 下一单元 (2026-08-13 起, 路由词「doc 轨 开始任务」落在这里)

C1 → U1 → U2 三个单元已闭环, **doc chunk 生产已可检索可答题**。剩余候选按依赖顺序:

| # | 单元 | 前置 | 一句话 |
|---|---|---|---|
| 1 | **判库欠账** | 无 | `q15`/`q17`/`q53` 在生产 `auto` 下拿不到 doc chunk (判库损耗 10pt)。**要修得改 router 不是改检索** —— 而 router 刚在 U2 动过, 再动必须重跑 181×3 闸 |
...
| 3 | **`both` 判库档** | 无 | 生产已启用 doc 通道而 `both` 下 context = 8+8+8, **三把尺子一条都不覆盖它** |
| 4 | **答题侧仪器** | 无 | 本单元实测全距 6.25pt > 声称效应 2.8pt ⇒ 任何 3pt 级结论都需要先把尺子做准 (空臂 + ON-ON 是最低配置) |
| 5 | **C2** (933/212 页表单 PDF) | U2 已完成 ⇒ 解闸条件 1 满足 | 仍需 §U3 的条件 2 (独立勘察 U3-pre) 与条件 3 (价值探针 5-10 页) |

**没有默认下一个** —— 上面五个互相独立, 由用户点单。
