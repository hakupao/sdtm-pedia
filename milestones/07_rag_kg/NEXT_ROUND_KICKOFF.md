...
**2026-08-11 补一句 (§2.A2 已 DONE)**: 零 LLM 的配额扫描证明了**配额安全** (`cap=2`/`cap=3`
全集零回归, q38 修到 1.0) 与 **`cap=1` 确定性有害**。但**"挤占是否伤害答案质量"仍然未知** ——
source recall 测的是 gold 召回, 不是答案。**§2.A′ (换锚重做层②) 的紧迫性没有下降。**
...
**剩下的**: q38 要靠**去重/配额** —— 见 §2.A2。**2026-08-11 全集实测**: `cap=2`/`cap=3` 让 q38
达 **1.0000** 且全集零回归; `cap=1` 虽也让 q38 满分, 但**确定性有害** (3 题回归), 不要用。
**注意 q38 在生产默认配置下仍是 0.3333** —— 本轮没有改生产。

### A2. ✅ **DONE 2026-08-11** — 零 LLM 的配额 x source recall 实验

> **结论**: 落在三种预设结果的"**净升**"一支, 但**收益 n=1**。
> `cap=2` / `cap=3` 在 140 题上**零回归**且把 q38 修到 1.0 (全集 `0.9917 → 0.9964`);
> `cap=1` **确定性有害** (q08/q37/q127 回归), 这条子档廉价关掉。
> **拿到确定性证明的是"配额安全" (n=139 零回归), 不是"配额对全集有正收益"** ——
> 尺子只剩 0.83pt 余量, 且唯一升的那题 (q38) 正是配额的设计出发点, **不是独立确认**。
> **本实验没有回答"挤占是否伤害答案质量"** —— 那仍是 §2.A′ 的事。
> 收口证据 `sdtm-rag/evidence/checkpoints/cap_recall_sweep.md` (含 §5 六类限制 +
> §5.5 规则 D 只完成一半的点名)。工具 `eval/cap_recall_sweep.py` (判定规则先于数据写死)。
> 5 个独立进程 (实现方 3 + 抽检方 2) 逐题逐档 560/560 全同。**生产未改**, q38 在生产默认
> 配置下**仍是 0.3333**。

<details><summary>原任务定义 (2026-08-09 写, 留作对照)</summary>
...
**存量债的实测结果 (2026-08-11)**: 那条债**本轮没有兑现** —— 新加的"影子信号"
(分数没动但 `source_hits` 集合变了) 三个档位**全部 0 题**。⇒ 本轮的"零回归"不是 OR 组撑出来的。
**但这条债本身没有消失**, 下次动检索仍要带上这个影子信号一起看。
...
17. **"派了审查" ≠ "审过了"** (2026-08-11 新立)。本轮按规则 D 派了两方独立 agent,
    **审查方 (critic) 零产出、抽检方 (scientist) 工件齐全但never回传报告** (催两次无应答)。
    实现方只能事后读它留在 `/tmp` 的工件并复跑其 harness ——
    **可执行工件救回来了, 文字判断 ("这条限制是真是粉饰") 没救回来**。
    ⇒ **拿不到报告就当那一环没发生, 在收口证据里点名, 不许把"派了"写成"审了"**;
    且派 agent 时要求它**边做边落盘**(工件写文件), 不要只在最终报告里给结论。

18. **新加的"取证信号"当场补断言, 否则它就是装饰** (2026-08-11 新立, 硬规矩 16② 的具体化)。
    本轮的影子信号 (`hidden_loss_shadow`) 是写完单测之后才加的函数, **没补测试** ——
    抽检方的变异测试把它改成恒返回空, **22 条里当时的 17 条全绿**。
    "三档全 0 题"那句话当时没有任何闸保护。**凡新增一个用来"证明没出事"的信号,
    先写一条"它出事时会红"的断言。**
...
**2026-08-11 配额 × source recall 轮新增** (全表见
`sdtm-rag/evidence/checkpoints/cap_recall_sweep.md` §5):

- **"配额安全"是全集结论, "配额有收益"不是** —— 上行分辨力只覆盖 q38/q126 两题
  (余量 0.83pt), 且唯一升的 q38 正是配额的设计出发点。**+0.4762pt 100% 来自 q38 一题。**
- **仍然没有回答"挤占是否伤害答案质量"**。source recall 的改善**不得**读成答案质量的改善。
- **判据对 75/140 题部分失明** (`s1_injected > 0`)。全部可观测效应落在 `s1_injected == 0` 的 65 题里。
- **q38 在 `cap=2` 下 context 从 15 段缩到 5 段** (其余 139 题保持满 15 席)。
  它的融合候选池 43 条里 **40 条 (93%) 是 §DOMAIN**, 只有 4 个不同 section, 补不满。
  **这个缩减是否伤答案, 没测。**
- **`cap=2` 与 `cap=3` 均值完全相同** —— 本尺子分不出这两档谁更好, **选档不能靠这份证据**。
- **`fuse_out=60` 是实验配置不是生产配置**。A 臂逐位等于生产证明了"放长输出 + cap=None"中性,
  但 **Task 7 上生产时"放长融合输出"要单独过闸**。
- **规则 D 只完成一半** (见硬规矩 17): 设计层面的对抗性复核**缺席**, 抽检方的文字判断**缺席**。
...
| **§2.A2 配额 × source recall 收口证据** | `sdtm-rag/evidence/checkpoints/cap_recall_sweep.{md,json}` |
| 同上 工具 (判定规则先于数据写死) | `sdtm-rag/eval/cap_recall_sweep.py` (`--summarize` / `--fused-profile` 零 LLM) |
...
```bash
cd sdtm-rag
.venv/bin/python -m pytest -q                                   # 980 passed / 0 F / 0 E / 0 S
