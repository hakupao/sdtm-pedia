...
| CDISC | hybrid + S1 | **95.71%** | **section 级判据** (`路径#节$`); 旧口径 98.93% 已作废但保留说明 |
...
CDISC 新旧口径同理。引用 95.71% 必须写明 "section 级判据"; 引用 81.07% 必须写明 "hybrid-only"。
...
### D. 两个新浮现的靶子 (本轮暴露, 未排期)
1. **study `form_overview` 类别仅 50%** (v2 实测, S2 开)。失分 6 题全是 v2 新题, 继承题 0 失分。
2. **CDISC 那 4 道假命中背后的真实检索缺陷**: S1 对 VARIABLE_INDEX 用文件内 cosine 选块, 而该文件的
   结构化单行 chunk 对自然语言问句近似语义噪声。**正解是按 CT 码 / 变量名字面定位 section**。
   修完这 4 题应能重回满分, 且**那时的满分是真的** —— section 化只是把缺陷暴露出来。
