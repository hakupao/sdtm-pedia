...
| 13 | **§0.5 CORRECTION 2026-05-05**: B-01 batches 02-04 ALREADY atomized model/02 + 03 + 05 (596 atoms total: 244 + 160 + 192) | `grep -oE '"atom_id":\s*"md_model[0-9]*"' md_atoms.jsonl \| sort -u` (含 `\s*` 兼容 space-format) | ✓ md_model01/02/03/04/05/06 全 6 文件 atomized |
| 14 | post correction: B-03 真实 scope = **127 files** (130 - model 02/03/05 = 127) | INDEX(195) + ROUTING(211) + VARIABLE_INDEX(2005) + domains × 124 = 127 | ✓ 127 |
| 15 | post correction: B-03a sub-cycle = **0 batches** (model 余 已全 done) | n/a | ✓ B-03a SKIP |

**Drift 校正记录 (B-02 retro / progress recovery_hint vs 实测)**:
- retro/progress 表述 "余下 model + 126 domains" → **实测 0 model + 124 domains** (post §0.5 correction 2026-05-05; B-02 retro G-B02-4 默认 126 是误)
- B-01 retro 表 "model 02/03/05/06 atomized" → **实测 model 01-06 全 atomized** (B-01 batches 01 atomized model/06 + batch 02 model/02 + batch 03 model/03 + batch 04 model/05; model/01 + 04 atomized 渠道 TBD 但 md_atoms.jsonl 含 72 + 24 atoms 已 evidence)
- 原 §0.5 行 4 + 6 grep 用 no-space format `'"atom_id":"md_..."'` (无空格) 漏数 B-01 batches 02-04 with-space format atoms (Python `json.dumps()` 默认 separators 含空格); 行 13 修复 grep 用 `\s*` 兼容
- B-03a 自治连跑 attempt 2026-05-05 PASS 2 batches (model/03 160 + model/05 192 atoms) 但因 §0.5 drift 后 atom_id collision detected → 用户 ack Option A revert (`evidence/failures/B-03a_drift_attempt/FAILURE_REPORT.md`)
...
## 2. B-03 Scope (post §0.5 correction 2026-05-05)

~~`model/` 余 3~~ (B-01 已 done 全 model) + `domains/` 余 124 + top-level 3 = **127 files** / **11,085 source lines**. 自检: B-02 close 11 files + model 02/03/05 (B-01) 3 + B-03 真实 scope 127 = 141 = PLAN §0.2 in-scope total.

| 类别 | Files 余 | Lines 余 | 估 atoms (range) |
|---|---|---|---|
| ~~`model/` (02/03/05)~~ | ~~3~~ → **0** (B-01 batches 02-04 done) | ~~784~~ → 0 | ~~~470-550~~ → 0 |
...
| **合计** | **127** | **11,085** | **~7,000-9,800 atoms 估** (refined post §0.5 correction; was 130/11869/7500-10500 pre-correction) |

post B-03: md_atoms.jsonl 累计 ~9,900-12,700 atoms / 141 files (out of 141 in-scope = **100% 文件覆盖 milestone**). P2 phase exit gate.

---

## 3. Sub-cycle 结构 (B-03 三段 → post §0.5 correction = 2 段)

B-03 比 B-02 大 ~3x atoms / ~5x files / ~30-60 batches estimate. 拆 3 sub-cycle 明确 boundary + retro 节奏 + cumulative audit 触发点:

### ~~B-03a: model/ 余 3 文件~~ → **SKIP** (post §0.5 correction 2026-05-05; B-01 batches 02-04 已完成 model 02/03/05 atomization; 596 atoms in md_atoms.jsonl). 详见 `evidence/failures/B-03a_drift_attempt/FAILURE_REPORT.md`. **B-03 真实启动 sub-cycle = B-03b**.
...
**总估** (post §0.5 correction): **2 sub-cycle = ~136 batches / 7,000-9,800 atoms / 3-5 weeks** (B-03a 跳过, B-03b 9 + B-03c 127 = 136).
...
**post §0.5 correction 2026-05-05**: B-03a 全 SKIP (model 02/03/05 已 done by B-01 batches 02-04). B-03 真实 batch 序列从 INDEX 起.

| Batch | Target | Lines | 估 atoms | 切片? | atom_id 起始 | 预派发 sub-cycle |
|---|---|---|---|---|---|---|
| ~~batch_01..03~~ | ~~model/03 + 05 + 02~~ | ~~~ | ~~~ | SKIP | n/a | ~~B-03a~~ → SKIP |
| **batch_04** | `INDEX.md` | 全 195 | ~150-200 | 单 dispatch | `md_index_a001` | B-03b #1 (B-03 真实 1st batch) |
...
注: batch numbering 保留 04+ 起跳 (NOT 重新从 01 起) — 维持 audit_matrix.md / trace.jsonl historical integrity, 同时 reflect B-03a 失败 attempt 已 archive.
...
| 用户说 | session 动作 |
|---|---|
| `P2 bulk B-03 开始任务` | 读本 kickoff (umbrella), 报告下一 batch (post §0.5 correction = batch 04 INDEX), 等 ack |
| ~~`P2 bulk B-03 batch 01..03 开始任务`~~ | ~~B-03a SKIP~~ (model 02/03/05 已 done by B-01) |
| `P2 bulk B-03 batch 04 开始任务` | 进 `P2_B-03_batch_04_kickoff.md` (TBD) dispatch writer for `INDEX.md` (B-03 真实 1st batch) |
| `P2 bulk B-03 batch NN 开始任务` (NN=05..~139) | 进对应 batch_NN_kickoff.md (按 §4 表逐个写, 派发前一刻完成 §0.5 grep checksum 含 `\s*` 兼容) |
| ~~`B-03a 闭环 mini-audit 开始任务`~~ | ~~SKIP~~ (B-03a empty post correction) |
...
| 用户说 | session 动作 |
|---|---|
| ~~`P2 bulk B-03a 自治连跑 直到 mini-audit PASS`~~ | ~~SKIP~~ (B-03a empty post §0.5 correction; failed attempt 2026-05-05 archived) |
| `P2 bulk B-03b 自治连跑 直到 mini-audit PASS` | **post §0.5 correction**: 自治执行 B-03b 全段 = INDEX + ROUTING + VARIABLE_INDEX 7-slice (~9 batches batch_04..12) → mini-audit → commit + push + 退出. **跨 VARIABLE_INDEX slice atom_id 连续性** 须 enforced. **B-03a precondition 自动 satisfied** (B-01 已 done model 02/03/05). |
