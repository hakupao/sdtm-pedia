# SDTMIG v3.4 --- Representing Relationships and Data (Section 8)
...
- Section 8.7, Relating Subjects, describes representing collected relationships between persons, both of whom are study subjects. For example: "MOTHER, BIOLOGICAL"; "CHILD, BIOLOGICAL"; "TWIN, DIZYGOTIC".
...
Supplemental qualifiers are represented as separate SUPP-- datasets for each dataset containing sponsor-defined variables (see Section 8.4.2, Submitting Supplemental Qualifiers in Separate Datasets).
...
- Findings interpretations that should be added as an additional test code and result. An example of this would be a record for electrocardiogram interpretation where EGTESTCD = “INTP”, and the same
EGGRPID or EGREFID value would be assigned for all records associated with that ECG (see Section
...
Tmax, half-life) and elimination rate constant; the structure is 1 record per parameter per analyte per reference time point per subject.
