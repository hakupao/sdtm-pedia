...
> Version: **v1.0 (2026-04-24, user ack'd)** — spec table 策略决定 Option A
> 父 PLAN: `.work/06_deep_verification/PLAN.md` v0.5 (user ack'd 2026-04-24)
...
> Changelog: v0.1-DRAFT (2026-04-24) → **v1.0 (2026-04-24 user ack'd, 三决策 O10/O11/O12 bundled ack, 即开始作业)**
...
| 总 batch 数 | ~55 batch |
| Writer 池 (v1.0 O-P1-07 resolution X' 2026-04-24) | **2-type alternation only**: `oh-my-claudecode:executor` ↔ `oh-my-claudecode:writer` (唯一同时满足 action-oriented + Write tool + sonnet-grade 的池) |
| **绝对禁用 writer** | (a) `Explore` / `oh-my-claudecode:explore` / `feature-dev:code-explorer` (search 家族丢 JSONL 20%+, P0 failures/ 教训 + v1.2 prompt ban); (b) `oh-my-claudecode:document-specialist` + `architect` + `code-reviewer` 家族 (物理无 Write tool, Tools field = "All tools except Write, Edit"); (c) `planner` / `Plan` / `designer` 家族 (role 不匹配 + 非 action-oriented) |
...
**决策 (v1.0 user ack'd 2026-04-24 → Option A)**:
- **Option A** ✅ **ACKED**: P1 PDF spec 表也全量原子化作 TABLE_HEADER + TABLE_ROW, 与 `knowledge_base/domains/*/spec.md` 做 side-by-side diff (P4a/P5 阶段), 验证 xlsx 脚本产物 vs PDF ground truth 一致性
- Option B: 跳过 spec 表, 仅原子化 narrative 部分, 接受 xlsx 脚本 PASS 历史 — **rejected**
- Option C: 采样 spec 表 (每域抽 3-5 行) 做 sanity, 非全量 — **rejected**

**决策理由**: 高 stakes + xlsx 脚本亦 AI 输出, 字面校验有价值. Batch 1 已按此策略跑完 (TOC + intro 域不含 spec 表, 纯 narrative/CROSS_REF), 后续 domain 章 (ig34 p.100+) 开始触发 TABLE_HEADER/TABLE_ROW 大量产出. Expected atom density ↑ 30-40% over batches 含 spec 表的页段.
...
### B.2 Batch roster 轮换 (Rule D §8 + v0.3 F-4 + v1.0 O-P1-07 resolution X' 2026-04-24)

**2-type alternation** (writer 池 = executor + writer only, 见 §0):

| Batch 序号 | Writer subagent_type | 实测 |
|---|---|---|
| 01 (奇) | `oh-my-claudecode:executor` | ✅ done 2026-04-24 (326 atoms) |
| 02 (偶) | `oh-my-claudecode:writer` | ✅ done 2026-04-24 (323 atoms, 9/9 type 单批首达) |
| 03 (奇) | `oh-my-claudecode:executor` | 🔄 in_progress 2026-04-24 |
| 04 (偶) | `oh-my-claudecode:writer` | pending |
| 05 (奇) | `oh-my-claudecode:executor` | pending |
| ...N | 奇→executor / 偶→writer (alternation, hard) | — |

**连续 2 batch 禁用同 type** (硬, 本质 alternation 天然满足).

**Drift 校准 3 种 type 需求** (§C): 2-type 池下降级为 2-type cross-check, 阈值保 ≥80%, 若 2-type 一致率 < 80% 额外派 1 次 `general-purpose` (带 Write tool) 作 tiebreaker. **不** 用 document-specialist/code-explorer 作 drift 样本 (同 §0 禁用原因).
...
### E.2 Rule A 抽检 — 频率升级 (v1.1 2026-04-24 post-30-page-milestone, O-P1-09 Option 2')

**原 cadence (v1.0)**: 每 30 页 (~3 batch) 派独立 reviewer 跑 30-atom 分层独审.

**升级 cadence (v1.1)**: **每 1 batch 后** (~10 页) 派独立 reviewer 跑 **10-atom** 分层独审, 门槛 ≥90%.
- Trigger 升级原因: 30-page milestone Rule A 100% PASS (广度验证 robust) **but** drift calibration 3-way FAIL on sparse-cell TABLE_ROW verbatim (reproducibility 噪声). Rule A 加密作 reproducibility 风险的 safety net, 替代 v1.3 prompt immediate fix.
- Reviewer subagent_type 轮换池 (未烧 4): `oh-my-claudecode:scientist` / `oh-my-claudecode:tracer` / `oh-my-claudecode:architect` / `oh-my-claudecode:ai-slop-cleaner`. 池不够时可复用 `superpowers:code-reviewer` (已烧 slot #12) 但 phase 内不连续.
- Reviewer 任务: 对当批 10 原子 (分层 per atom_type 按出现比例, FIGURE/NOTE 强制至少 1 若存在) 独立对 PDF 原页校 verbatim + atom_type + parent_section + HEADING 字段.
- Output: `evidence/checkpoints/rule_a_batch_<NN>_verdicts.jsonl` + `rule_a_batch_<NN>_summary.md`
- **PASS 门槛 ≥90%** (≥9/10).
- <90%: halt P1, 读 fail 原子具体原因, 升 v1.3 prompt 或回炉该 batch.

**废弃原 30-page milestone 版**: 保留作历史 (evidence/checkpoints/rule_a_30page_*), v1.1 起不再 triggerd.

**Drift calibration cadence 不变**: 仍每 3 batch (~300 atoms), §C.1 不动.
