...
### 同日续 — C3: event 通道 precision 取舍单元 (Tier 1b 判别力闸)

复审要求 C3 "合成一个**带数据判据**的单元, 而不是各自反射性补齐"。本单元先做归因,
结论是**复审原议的两个半边都不该做**。

**基线逐层归因** (先归因再谈方案): Tier 1a **100%** (5/5) · Tier 2 **100%** (12/12) ·
**Tier 1b 2.7%** (220 返回 / 6 tp / 214 fp) · Tier 3 22.7%。**Tier 1b 一层占 85% 的
返回与 93% 的噪声** —— 全局 10.81% 基本由它一层决定 (259/28/10.81% 与 checkpoint
§6-15 逐位吻合, 独立复算)。清单长度分布 `{1:13, 2:3, 4:1, 14:1, 15:1, 18:1, 40:1}`,
噪声集中在那 4 个长清单 form。

**两个半边实测均为纯亏损**: 半边A 逐条相减 21/33 不变 → precision 9.06%; 半边B
名称→assignment 21/33 不变 → 9.43%; A+B → 8.07%。**一题都没多命中。**

**半边A 的诊断值得单独记**: 它要修的设计缺陷**真实存在** —— 实测 33 题共吞掉 66 条,
其中 **12 条确实在 gold 里**。但那 12 条**全部属于已经命中的题**, 所以在"每题≥1条
gold"这个口径下**这把尺子根本看不见这个缺陷**。⇒ 现在修 = 确定付 50 条噪声, 换一个
尺子量不到的收益。**判定: 不做**, 待口径换成条目级 recall 后重估。

**采纳**: `_MAX_ASSIGNMENTS_PER_FORM = 1` (与既有 `_MAX_CARDS_PER_MATCH` 同精神:
匹配集合太大 = 不具判别力, 整个跳过)。**命中 21/33 → 20/33 · 返回 259 → 50 ·
precision 10.81% → 54.00%**; 逐类只有 `event_form_assignment` 由 1/5 → 0/5, 其余五类
逐格不变 —— **repeating_rule 5/5 全保住, 说明 Tier 1b 的真实价值在短清单上**; 它服务
的另一类即便放任 214 条噪声也只有 1/5, 那 1 题更像撞上的不像能力。被淘汰形状: 按题
限流 K=1 (61 返回/44.26%, 各项均劣, 且取哪 K 条依赖 dict 插入序)。

**契约变更留痕**: 既有测试原用 2 条 assignment 的 form 断言"两条都在", 该断言在
`=1` 之后**不再成立**。已改写为 `..._lists_assignments_without_subtraction` (改用 1 条
assignment 的 form 演示同一意图: Tier 2 会减、Tier 1b 不减), docstring 显式记下 ——
**不是改绿, 是换契约**。

**四条限定** (引用数字须同框, 证据 §4): n=33 且自出尺子, 与基线只差 1 题 (margin 在
33 题上就是噪声量级) / `resolve_events` 零生产调用方, 今天不影响线上 (卡片侧
`resolve()` 与 `_form_assignment_index` 无交集, 引用面 grep 确认) / **口径对条目级
recall 失明**, 半边A"无收益"完全由此导出 / 33 题外泛化未测, `=1` 意味着通道**永远
无法**回答"这个 form 被分配到 2 处以上"。

**实测**: `pytest` **1791 → 1792 passed / 0 failed`; **真实 `resolve_events`** 在 33 题
上 20/33 · 返回 50 · **54.00%**, 与 replica 预测**逐格一致** (replica 的结论不当实现的
结论用)。证据 `sdtm-rag/evidence/checkpoints/study_c3_precision_tradeoff.md`。

**给 C4 的输入**: 接线前的 precision 门槛现在有可比基准 —— **本通道当前上限约 54%**,
且在自出尺子上; 门槛若定在 54% 以上, **C4 现在就不合格, 应先停**。另 `event_timing`
0/5 与 `event_form_assignment` 0/5 是**两个整类答不出**, 接线不会改善它们, 只会把它们
的噪声送进检索。

### 同日续 — 砍掉 Tier 3 + 定 C4 准入门槛 (用户裁定)

**归因**: Tier 1b 收紧后, 全通道剩余 **23 条 fp 里 19 条 (83%) 出自 Tier 3** (层内
tp 5 / fp 19); **Tier 1a 与 Tier 2 在全 33 题上 0 fp**。机制: event/activity 可读名是
自然语言短语, 日文没有可用的词边界, 该层不做边界判定 —— 题面出现该短语即召回。

**移除后**: 命中 20/33 → **15/33** · 返回 50 → **26** · precision 54.00% → **84.62%**。
收紧(而非移除)的曲线均劣于直接移除的性价比 (名称最短 ≥4 → 59.09% / ≥8 → 76.67%)。

**代价与收益同框 — 逐类**: `item_collection_scope` 5/5 与 `repeating_rule` 5/5 不变,
`conditional_event` 4/7 → 3/7, **`oid_name_mapping` 6/6 → 2/6 (本次最大单项损失)**。
丢的 4 题全是**「给名称找 OID」**方向 —— 只由 Tier 3 承担。⇒ **砍完之后本通道只认
标识符字面**: 题面里必须出现 OID 才响。行为画像: 单题返回量 中位 **1** · 最大 **7** ·
**14/33 题返回 0 条**。

**连带清理**: `_name_index` / `_MIN_EVENT_NAME_LEN` / `qn` 随层移除 (零引用 = 死代码,
与 C2 刚立的纪律同一条)。

**两处契约留痕 (不是改绿)**: ① 两条正面测试 (名称能查到) 合并改写为**一条反面测试**,
写明这是**能力删除而非重构**; ② 防挤占测试原用 Tier 3 洪水, 该层移除后**场景再也构造
不出来 ⇒ 断言恒真 = 装饰闸**, 已换洪水源为 "一个 item 的 collect_scope 覆盖 60 个活动
→ Tier 2 单层产出 60 目标", 守的是**拼接顺序**不变量, 变异实测红/还原绿。

⚠ **踩坑记账 — pyc 假还原 (第二次)**: 首次变异验证时源已还原但 `__pycache__` 仍供旧
字节码 (该变异只交换元组顺序, **字节数完全相同**), 造成"还原后仍红"的假象, 我一度误判
是测试写错了。清 `__pycache__` 后基线绿/变异红/还原绿。**本仓 U5 抽检 B 已记过这个坑**
("harness 假阴性与 pyc 假还原两坑踩实") ⇒ 新规矩: **变异测试每轮必清 `__pycache__`**。

**C4 准入门槛 (本单元最终产出, 证据 §8)** —— 三条全过才接线:

| 门槛 | 现状 |
|---|---|
| G1 精度: precision ≥ 80% 且每命中题平均 fp ≤ 0.5 | **84.62% / 0.21** ✅ |
| G2 不回归: study golden v2 卡片侧**三遍**逐题 Δ0 | **未测** ⬜ |
| G3 需求: ≥10 条真实用户提问能被本通道回答 | **0** ❌ |

**G1 阈值是事后设定** (看到 84.62% 之后才定), 引用必须带这个限定; 机械理由 = 4:1 tp:fp
与"平均每次注入不到半条噪声"。

**结论: precision 不再是 C4 的阻塞项**, 阻塞项换成 G3 与未测的 G2。**且 G3 比砍 Tier 3
之前更关键**: ① 通道已变成只认 OID 字面, 真实用户自然语言提问里**打出 OID 的概率至今
无任何数据**; ② 33 题由知道 catalog 的人出, **19/33 的触发率是尺子上的, 不是世界上的**。
⇒ 若真实提问里几乎没人打 OID, 接线与否对用户无差别, C4 的正确动作是不做。**这个问题
只能由真实提问数据回答, 不能由再出一版尺子回答。**

**实测**: `pytest` 1792 → **1791 passed / 0 failed** (净 −1 = 两条正面测试合并为一条反面);
真实 `resolve_events` 15/33 · 26 返回 · **84.62%**, 与 replica 预测逐格一致。

**用户同日确认**: Plan C 的两份 PDF (933 页 workflow + 212 页 Annotated aCRF) **维持
DROP 判决** —— 原判在 `c2_pre_survey.md` §8 (相对 workflow 三表零增量且严格劣于它们),
本次经用户明确同意, 该判决**由前 session 的技术判定升为已确认决策**, 不再重开。

### 同日续 — 补 C1 L1 卷首覆盖缺口 + 新增全文覆盖闸 (用户裁定)

**先把数字摆正**: L1 记的 66,200 字符 (27.42%) 里 **3/4 是目录** (49,320) —— C1 原判
"目录排除合理", 本轮经用户裁定维持。**真正的实体缺口是 16,880 字符 = 全文 6.99%**
(卷首 p1–10 的 16,302 + 首锚点页 p17 前段 578), 已补为 **11 个 chunk**
(`study_st01_docs` **114 → 125**)。

**根因修复 (本单元核心, 比补内容更重要)**: `assert_partition_complete` 的口径原文是
"必须逐字等于**首锚点行起**的全文" ⇒ 首锚点**之前**的区域不在它的口径内, 所以 C1 那轮
**三把闸全绿而缺口存在**。新增 `assert_full_coverage`, 口径改为"**全文减去显式声明的
排除页**"; 排除页必须由调用方传入且有形态依据, **不接受"没覆盖到就算排除"的事后追认**
—— 那等于把闸的口径交给缺陷自己定义。不修闸, 同类盲区会在新位置复发。

**目录识别用形态判据, 不用硬编码页号** (页号写死后换一份 PDF 会静默排除真实正文而闸
照样绿): 编号行 ≥8 且 点线行 ≥3。**全 113 页实测**: 只有 p11–16 满足前者 (正文页最多
2 条编号行), 且这 6 页各有 **25–46 条点线行**, **107 个正文页恒为 0** —— 两条各自都已
能分开, 合取是为了让"某页恰好排版成多列数字表"也不被误判。

**卷首按页切的依据**: p1–10 实测**编号行 0 条、可识别标题 0 条**, 锚点切分器无处下手;
页是抽取器本来的单位。token 实测单页 **169–1,907** 安全, **整块 10,034 超 8,191 上限**。
编号用 `front{页号:02d}`, 与编号节命名空间不相交, 不污染 `check_numbering`。

**验收**:
- **既有 114 个 chunk 逐字节不变** (`cmp` 逐个比对: 不变 114 / 变了 0)。预算由 8104 变
  8100 (卷首编号更长, frontmatter 开销略增) 但二次切分结果 `{8.2:3, 22.1:2}` 未变。
- 11 个新块正文合计 **16,880 字符**, 与 L1 记录逐位吻合。
- collection: `study_st01` 959 不变 · `study_st01_docs` **114 → 125**。

**回归 (doc 侧 30 题, retrieval-only, 零 LLM)** —— ⚠ **默认尺子饱和, 不能拿它交差**:

| 尺子 | 基线(114) | 终态(125) | 判读 |
|---|---|---|---|
| k=15 默认 | 1.0 ×3 | 1.0 ×3 | ⛔ **完全饱和, 零判别力** |
| **k=5 判别力尺子** | **0.8833 ×3** | **0.8833 ×3** | ✅ 未触顶 |
| 三遍内部稳定 | 0 题波动 | 0 题波动 | |
| **前后逐题差异** | — | **0 题** | ✅ |

k=5 是 U1 checkpoint 指定的判别力尺子 (当时 88.33%, 本轮复现同值)。**基线由本轮自己
重灌 114 版跑出**, 不引用 U1 期存档 run (中间 U2 接线等改动可能影响可比性)。

**测试**: 1791 → **1798 passed / 0 failed / 0 error / 0 skipped**。

**已知限制 6 条** (证据 §6), 最重要的一条: ⚠ **"补上了"是覆盖口径, 不是"能答出来"口径**
—— doc 侧 30 题 gold **全部出自编号章节, 没有任何一道在考卷首内容**。本轮的零回归只
证明新块**没有挤占**既有检索, **不证明**它们检索得到或有用。要证后者需针对卷首出题,
本轮未做。其余: 目录仍不可检索 / 卷首按页切可能切断跨页的 synopsis (与 L6 同族, 未评估)
/ **`docs/` 目录指纹 `0f13797f…` → `1d2bc9eb…`, U1 §7 复跑命令里的 shasum 断言将不再
匹配 (预期结果非缺陷)** / L9 (PDF 文本层是否忠于版面) 无改善 / 形态阈值仅在这一份文档上
验证过。

**操作留痕**: 重灌前后各停/起一次 launchd `com.sdtmrag.api` (delete_collection 与
lifespan 持有句柄冲突, 既有 Ruling); 终态服务 HTTP 200 正常。

### 同日续 — 公开仓历史改写 + force push (用户裁定)

`main` **876ce4b → 9a13dd2**。7 个真实标识符在 **930 commit / 6,927 blob / 全部提交信息**
中归零, 两个 CJK label 的 bigram 残留 0 (无法重构)。**HEAD 树 `b0d039449bf1` 改写前后
逐位相同** —— 只清历史, 内容零改动。

**范围是逐条判出来的, 不是照单全收**: 扫描器报 **19 个 needle**, 只清 **7 个**。其余 12 个
全部落在 `knowledge_base/`、`06_deep_verification` 的 CDISC PDF 抽取 atoms、`release/`、
`ai_platforms/` 等公开内容里 —— **照单全清会毁掉公开知识库**。另有 2 个已知为真但故意不清
(3 字符 form OID 同时是公开肿瘤学缩写; 4 字符 OID 早在闸的 ALLOWLIST 里注明用作通用临床
判读用语)。新查出的泄漏点两处: `U1_RESUME.md` 有一行"高危 form"后**逐个列出真实 form OID**;
commit `a36159bb1` 的**提交信息**在说明"剔除 fixture 真实 OID"时把那个 OID 写了出来
(⇒ `--replace-message` 必须一起开)。

⚠ **第一次改写失败并回滚 —— 必须记账**: v1 改坏了 **19 个公开 CDISC 知识库文件**,
needle#7 (4 字符 OID) 恰好是标准变量名 `SRGSTIND` 的**子串**, 被替换成 `SREDACTED_OID_07IND`。
**错因: 我用闸的词边界判据验证范围, 却用不感知边界的 `--replace-text` 执行。**
尚未推送, 从镜像备份**整仓回滚**(HEAD 与树逐位还原) 后重做。

**v3 的三条约束 (v1 教训直接转化)**: ① **顺序无关** —— 每条字面量互不重叠 (v2 想用整行,
但同一行上多个 needle, 裸替换先命中会让整行失配); ② **外溢者用带上下文的形态** —— 实测
7 个里 **6 个零子串外溢**, 只有 needle#7 外溢, 改用带反引号形态; ③ **bigram 用最小上下文
子串** —— 全局替换 2 字符片段会改坏大量日文正文 (实测一个片段出现在 **112 个路径**)。

**改写前干跑 (v1 就是没做才翻车)**: 4 个目标路径的**全部 27 个历史版本**模拟应用 ——
第一次发现 **1 个版本仍可重构 label** → 补 1 条字面量 → 第二次 **0 残留**; v1 踩坏的 5 个
样本模拟**全部不变**; 当前 main 树 **4,296 个文本文件 0 个会被改动**。

**改写后本地 + 远端各验一次** (远端 = 从 GitHub 重新 mirror clone 后扫描), 结果一致:
needle #1-#6 全 0 · needle#7 **独立 token 0 处** (4 次裸子串全部嵌在 `SRGSTIND` 内) ·
提交信息全 0。`pytest` **1798 passed / 0 failed`; 红线闸 rc=0。

**做不到的部分 (证据 §6)**: GitHub 仍保留改写前的孤儿对象, 彻底清除**需联系 Support purge**
(本轮未做) / **`refs/pull/{1,2}/head` 永久保留, force push 删不掉** —— 本次运气好, 两个 PR
都是 2026-04 CDISC 阶段, 其 9 个命中逐条查证全是公开术语撞车 / 已有 clone 与 fork 不受影响 /
本地镜像备份 `~/MyProject/_sdtm_pedia_backup_20260826_224423.git` (68M) 仍含旧历史, 是回滚
保险, 删除时机由用户定。

---

## 2026-08-27 C4 接线三闸 — G2 对照臂刷新 + G3 判据预登记 (零生产改动)

- **触発**: 用户问「目前 planc 的进度是什么, 下一步建议怎么做」

### 先纠正一个误读: Plan C 已整线关闭

**Plan C = study 轨有损轨** (933 页 workflow PDF + 212 页 Annotated aCRF → markdown 入库),
**2026-08-25 两份均判 DROP, 预登记 S2 触发, 整线关闭**, 已从 `DOC_TRACK_KICKOFF.md` §0′
候选表移除。现在在动的是它的**替代物** —— ConfigReport 三个未解析 sheet 的确定性解析
(`resolve_events`), **不属 Plan C** (性质不同: 零有损、零 LLM)。

Plan C 那一轮最值钱的产出是**自己推翻自己**: 初判 C2-a = RELATION, 同日发现基线定错
(用了 `catalog.json` 这个**下游产物**而非源数据), 加上 workflow 三表重测后 16 页 coverage
21.46% → 49.10%, 残余 81 项逐项可解释、**零真增量**, 改判 DROP。教训已入档
(`c2_pre_survey.md` §8-7): **增量必须相对源测, 不是相对当前产物测**。

### 本轮做的两件事

**① G2 对照臂刷新 (不是 G2 本身)**

87.50% 那个参照物取自 08-26 事件层单元闸 D, 此后 HEAD 又动过三处 —— `適用範囲` 改标签
并重灌索引 (231/961 张卡片正文变化) / Tier 3 移除 / L1 卷首 +11 chunk (114→125) ——
**每一处在原理上都可能动卡片侧检索, 拿旧值当参照物不成立**。HEAD 重取三遍:

| 项 | 结果 |
|---|---|
| `source_recall_avg` | **0.875 ×3** 逐位相同 |
| 三遍逐题一致性 | 51 个 id **0 题波动** |
| 对照 `v2_baseline_s2on.json` | **逐题 Δ0**, id 集合完全相同 (无新增无删除) |
| 计分 | scored 48 / out_of_scope 3 / 非满分 6 ⇒ 42/48 = 87.50%, verdict PASS |

⇒ **三处改动对卡片侧零漂移**, 对照臂可继续用。

⛔ **这不是 G2**。G2 判据是「union-add **之后**卡片侧三遍逐题 Δ0」, **接线不存在就没有
被测对象**, G2 状态仍是**未测**。且刷新命令走卡片侧路径 (`--study-lookup
--collection study_st01`), **不含生产 docs 引擎 (U2 接线的 `StudyCorpusEngine`),
看不见 docs 侧挤占** —— 选它是为了与闸 D 对照臂同口径可比。

**② G3 判据预登记 (数据未看, 单独 commit `74636d5`)**

同款做法见 `c2_pre_survey.md` §0 —— 那一轮基线定错导致误判, 正是本轮把判据提前钉死的
直接原因。git 历史即证据。

- **PASS 充要条件两条**: `resolve_events(原话)` 返回非空 **且** 返回集合里至少一条是该
  提问真正要的答案。**第 2 条由用户逐条勾选, 我不自判** (自己出题自己判会重演本仓多次
  出现的「尺子饱和」闭环)。
- **无条件记录三个数**: 是否含 OID 字面 / 命中靠哪一层 / 返回条数。
- **自毁条款**: **S1** PASS<10 ⇒ G3 不达标、C4 不接线 (沿用 C3 §8 原阈值, 不改) ·
  **S2** PASS 里 ≥8 条含 OID 字面 ⇒ 须显著标注「适用面依赖用户打 OID」, 不得读作需求
  已证实 · **S3** 只收真实回想起来的全部, **不足 10 条如实记不足比凑够更有信息量**。
- **S4 收题期防偏** (后补, `37978ff`): 收题期间**不做即时判定**, 每收一条只登记、不告知
  能否被通道答出, 收满后一次性跑。理由: 用户已知「通道砍掉名称层后只认 OID 字面」,
  边收边看结果会让后收条目向「能答出的形状」漂移, G3 就退化回尺子。**追加时点已在条款
  内声明** = 首次 commit 之后、任何一条真实提问到达之前, 未触 §0 禁令但不静默并入 S1-S3。
- **§6 写清本判据看不见什么五条**: PASS 不证明接线后能活下来 (那是 G2) / 样本是单一提问者 /
  S3 无法程序化验证 / PASS 集合天然偏向含 OID 是**通道能力边界**不得反读成用户提问分布 /
  全部判定离线 (`resolve_events` 零生产调用方)。

### 查清的一件事: G3 现在不是「没收集够」, 是「没有采集管道」

实测三条: `dogfood_failures.md` **不存在** (`/api/flag` 有 2 次调用但落在 override 路径);
launchd 日志 **33 次 `POST /api/ask` 全是冒烟/自测**; **问题文本任何地方都不落盘**
(`server/main.py` 无提问日志)。

⇒ 所以 G3 有两条路: (a) 用户从真实工作里回想 10 条 (**已选**, 慢速收集, 路由词「G3 收题」);
(b) 先建脱敏 query log 再等真实流量。(b) 数据更硬且能顺带给「尺子自出」这个全仓共同根因
第一份真实分布, 但慢一个数量级。
...
| 闸 | 现状 |
|---|---|
| G1 精度 ≥80% 且每命中题 fp ≤0.5 | **84.62% / 0.21 ✅** (⚠ 阈值事后设定, 引用须带此限定) |
| G2 卡片侧三遍逐题 Δ0 | **未测 ⬜** (等接线; 对照臂已刷新) |
| G3 ≥10 条真实提问 | **收题中 ❌** |

⛔ **本轮零生产改动、零接线**, `resolve_events` 仍零调用方, 全部 precision 数字是离线测量。

**红线**: `oidscan_evidence` CLEAN (1638 needle / 3 次扫描 / 0 命中); 收集台建于
`data/study/` (gitignore 覆盖, 已 `git check-ignore` 复核)。

**账务观察 (未处理)**: ① `DOC_TRACK_KICKOFF.md` §0′ 候选 #2「L1 卷首 chunk」已于 08-26
做完, 候选表未划掉; ② `milestones/07_rag_kg/_progress.json` 内容停在 2026-07-10 (SP6),
U1-U6 / 事件层 / C1-C4 全部单元都没更新它 —— Chain B 的这一环在 doc 轨/study 轨已事实
停用, 状态实际由 PROGRESS.md 单点承担。本轮**按既有先例未复活它**, 是否重启由用户定。
...
## 2026-09-01 · B3' eval `--web-search` lever + 兄弟洞 R3 + 预防规则 5/6

> 分支 `fix/eval-web-search-lever` (基线 main `1fa6477`, 1865 passed → **1881 passed**)
> 派单/报告/复审 全留在 `.superpowers/sdd/2026-09-01-b3-eval-web-lever/`

### 起因

联网参考通道分支合并后留下的唯一 ⛔ 硬约束 (spec §10.1 B3'): eval 把
`web_search_enabled` 写死 `False`, 而生产每次请求都带 Rule 9 ⇒ 140q / study 48q
的数字描述的是一个**生产不跑的构型**。

### 做了什么

1. **B3' lever** — `eval/run_eval.py` 加 `--web-search`, 两处注入点 (cdisc 主引擎 +
   联邦 `study_levers`) 共用同一个 `args.web_search`; 回执与 summary 均落盘且**记引擎
   实收值 (事实) 而非 args (意图)**; 9 条新闸钉两方向 × 两注入点 + help 边界。
   help 与代码注释写死命名边界: 本 lever 只复现 prompt 构型, **eval 不联网** (工具循环
   只在 `/api/ask_stream`), 将来真接循环时必须用同一个 args 不得新开第二个 flag。
2. **R3 兄弟洞 (复核轮发现, 用户批准扩围)** — `prompt_guardrail` 同款病、同一文件、隔 8 行。
   修前**四格里三格全裸** (钉常量 True/False 各 1874 全绿), 可跑出**标着 `guardrail=ON`、
   实际全程 OFF** 的 140q 数字。已修 + 7 条新闸。详见 spec §10.1 **B5** 行。
3. **预防规则 5/6 进 `.work/meta/retrospective.md` §4** (session 启动必读件, 四条→六条)。

### 关键教训 (均已落进规则 5/6)

- **规则 6 初稿是错的**: 忠实转录了原始记录里"五条里有四条同一成因"的论断, 但那个论断
  本身没人验算过。复审逐条验算后推翻 —— 真正符合的只有 2 条, 而开的药只对 1 条有效。
  ⇒ **采纳一条归纳时, 归纳本身也要先验算**; 忠实转录 ≠ 已验证。
- **写下规则并不让你免疫它**: 为堵 `--web-search` 命名陷阱而新写的 help 闸, 自己就是裸
  `in` 扫全文 —— 边界句删光 + 锚点塞进无关 flag, 9 条全绿。是复审做变异做出来的。
- **变异验证本身会静默失效**: 实现者的变异脚本裸子串定位失败被 shell 吞掉, 拿未变异代码
  跑出"全绿" —— 而"全绿"恰好等于它要证明的假设的预期输出 ⇒ **假确认, 不是报错**。
  它自己复查发现并重跑。
- **一类缺陷极少只出现一处**: B3' 修完隔 8 行就有兄弟 (R3)。
- **行号会漂**: 规则 6 引的四个行号错了三个 (复审凭印象填、控制器忠实转录), 已改引函数名。
  同族形状 —— 只是把"未标 commit 号的数字"换成了"未核的行号"。

### 状态

⛔ **B3' 硬约束维持不解除** —— `--web-search` 默认 off, 解除条件不变: **真以 `--web-search`
跑过一遍**。「flag 已加」≠「B3' 已修完」。
...
## 2026-09-08 (下午) — Chat UI 后续三件: 高频控件出齿轮 + 输出无上限自动续写 + 收尾杂项

- **高频控件出齿轮 (方案 A, 用户点名)**: 模型下拉 + 检索范围三 pill 从设置弹层移到输入框底部工具行, 齿轮只剩「行内出处」; `#model-warning` 落在输入框正下方。零 JS 改动 (控件全走 `$(id)`), playwright 加 3 条位置断言。Rule D 审阅 (opus) 抓出**本次新引入**的 `#to-bottom` 硬编码偏移在警示行可见时压输入框 ⇒ 改为锚在 `.messages-wrap` 底边 (`085c450`), 顺带消掉 §8 里「输入框 200px 时重叠」的老限制。截图 `screens/09-12`。
- **输出无上限 (用户裁定: 不要手动「继续」, 要服务端自动续写)**: 起因是真实用例「VS 用法 + 本研究映射」在 ~4k token 处硬截断且 UI 无提示 (已按 ⚑ 格式记入 `dogfood_failures.md`)。根因: 全部 `acompletion` 不传 `max_tokens`, `bedrock/converse/` 路径 litellm 不发 `maxTokens` ⇒ Bedrock 低默认值。修法两层 (`0ad22d5`/`1363f2b`/`5376dc2`/`810848f`/`0b2f737`): ① **每个 Router deployment 显式 `litellm_params.max_tokens`** (opus-5/sonnet-5 128k AWS 确认; gpt-terra/sol 128k 无一手文档、探针实测接受; DeepSeek 兜底保守值) —— 部署级而非每次调用, 回退到 DeepSeek 不会带着 Claude 的天花板; ② `finish_reason ∈ {length, max_tokens}` 且无工具调用 ⇒ 回灌 assistant 正文 + `CONTINUE_PROMPT` 续写, token 流进同一气泡, 兜底 8 轮只防死循环, `done` 报 `continue_rounds`/`truncated`; 前端 `continue` 事件 + 「自动续写 ×N」徽章 / 触顶警示, 存档可复原。非流式 `/api/ask` 同款 + 墙钟预算 (`request_timeout_s`) + 续写轮失败保底 (200 + 已生成正文 + `continue_error`)。
  - 探针 (进仓 `eval/probes/`, 需 `--yes-spend`): 8 deployment 全接受各自上限; 真截断在 opus-5/gpt-terra/DeepSeek 上均报 `length`; 端到端把上限压到 4000 ⇒ 19,310 字符 3 轮续完, 章节 1-10 连续, 接缝零重复 (**n=1, opus-5 英文**, 中文/其他模型待观察)。探针逼出一个 brief 没预见的缺陷: 推理耗尽预算 ⇒ 一轮零可见输出, 回灌空 assistant 会被 litellm 丢掉并合并 user 块 ⇒ 模型自编续写起点 (`810848f` 红先绿修)。
  - Rule D: 审阅 (opus) 0C/5I/7M ⇒ 修复轮补墙钟预算、工具优先守卫测试 (变异从存活到打红)、`finish_reason` 守卫测试、非流式 usage `partial`、SSE 契约新字段形状、探针进仓; 复审 (opus) 8/8 ADDRESSED **Approve**。2051 → **2073 passed**, node 27。eval 的 `MAX_TOKENS=8192` **有意不动** (V-2 可比性)。剩余 Minor: Streamlit 提示文案把预算耗尽也叫「续写上限」; `model_used` 非流式只报末轮; 上下文窗口实际把轮数压在 8 以下。
- 生产两次由用户 `launchctl kickstart -k` 重载 (限流豁免 / 自动续写), 每次 curl 验证 health 200 + 静态 42/42 200 + `/api/info` 四模型齐。
- 杂项: `dogfood_failures.md` 入 `.gitignore` (运行时追加写, 本机保留); 局域网访问建议 = 改本地主机名后用 `bojiang.local:8000` (mDNS 零配置), 去 `.local` 需 IT 做 DNS A 记录 + DHCP 保留 (与 go-live 同一单); 日志显示 192.168.100.24 已在 LAN 内实际使用。
- 落盘: `sdtm-rag/evidence/checkpoints/{chat_ui_redesign_2026-09.md,autocontinue_2026-09.md}` + 子目录 (SDD 台账 / 终审 / 截图 / 研究报告)。

## 2026-09-09 — C2R: 表单 PDF 按需旁路 (画面判读通道, 默认 OFF) + 活動 OID 日语对照表 (L1)

- **起因**: dogfood 2026-09-09 10:49 (LB Form 身高体重题) 用户两条反馈: ① 答案里裸 OID 无官方日语 label; ② 卡片层答不全显示/时点类问题, 希望把两份 DROP 掉的表单 PDF「以人的视角」当画面看。用户与我对齐: 钥匙不是问题原文而是命中卡片的 OID; 触发看命中形状不看措辞; 抽页渲染成图走多模态; 附加通道不替代检索。Tier 2, PLAN `sdtm-rag/PLAN_c2r_pdf_bypass.md`, 进度 `_progress_c2r.json`。
- **S0 勘察 (零 LLM, G0 PASS)**: PDF 文本层 OID 几乎搜不到 → 改钥匙; 两份 PDF 均可按标题行**确定性切块** (workflow 110 块 == 110 条 assignments 1:1; Annotated 21 form 块 + 21 code-list 块), Annotated 941/959 item OID 可定位 (中位 1 页); 规则 A 12/12 页人眼 PASS; Bedrock 四 selectable model 4/4 接受图像 (110dpi ≈1.5k token/页)。⛔ 单键 activity 仅 76.6% 达线, 必须 (activity, form) 双键。证据 `evidence/checkpoints/c2r_s0_survey.md`。
- **L1 对照表**: 答题时在 study 段末尾附「EDC OID 対応表」(activity/form/event → 官方日语名, catalog 机械对译, `####` 无 `---` 与 chunk 区分, 禁止给它编 `[Source:]`) + prompt 规则「引用 OID 必附官方名, 查不到只写 OID」。**不改卡片文本** (避开 t4_step7 已实证的挤占回归); AST 函数级 diff 证检索链逐字节不变; golden v2 逐题 0.875 Δ0。Rule D 复审 PASS-with-nits → 4 MINOR 修 (对照表上提 `StudyCorpusEngine` 覆盖手順書章节 20/125 文件; 缺 name 字段容错; ruff; 消歧)。证据 `c2r_l1_oid_glossary.md`。
- **I2 通道**: `scripts/study/build_pdf_page_index.py` (块表 + item→页, 三条断言不过不落盘, PDF sha 入 meta); `server/pdf_context.py` (OID→页→`pdftoppm`→多模态 parts, 6 页上限, Annotated 占半预算, 同画面折叠可见化, 日语降级提示, 启动校 sha); `server/pdf_trigger.py` (R1/R2/R3 纯函数, R3 相关性下限 = via_lookup ∨ strong_hit ∨ 问题点名 study 表单 (页索引 names.forms)); `router.py` 两端点接线 + `pdf_pages`/`pdf_trigger` 字段; 7 个 `SDTM_RAG_PDF_*` 键, **默认 OFF**, 开着缺索引/PDF/pdftoppm = 启动失败。Rule D 复审 PASS-with-nits → M1 (sha 只记不校) / M2 (同画面去重静默丢被问活动, 110 块 69 块符合) 等 10 项全修。前端 `webchat/js/pdfpages.js` 三态展示 + SSE 契约闸 (变异验证)。
...
### 2026-09-14 (下午) — C2R N5 卡片层接地闸 + census 扩围 + prompt 规则 (闸 attempt 1 FAIL → attempt 2 压线 PASS; 复测四判据 PASS, ④ 字面 FAIL; N6 候选 CDISC 层出典塌陷)

- **起点**: N4 judge 首见卡片层捏造 (三道闸都不覆盖)。brainstorm 经用户批准, 用户选「测量 + prompt 一起做」。**勘误**: N4 审计「16 源 0 次」被 Writer/reviewer/judge 三方独立推翻 —— 该 OID 在同 run 兄弟卡片非表示列表里 (run json `text_preview` 300 字 grep 之误), 形态 = 移植 + 挂真出典; Erratum 附于 N4 审计末尾。
- **闸** `scripts/study/c2r_eval/check_card_grounding.py` (零 LLM; 与 N2 共用切分 helper, 抽出后 N2 逐条相同): 单元 = study 出典句; 实体 = catalog activity/event/item/group OID; 类 SOURCE_GROUNDED / SIBLING_NAMED / CONTEXT_MISCITED / UNGROUNDED / BAD_SOURCE / NEGATED / NO_ENTITY。首跑 56 文件标红 61 → 六条 pattern 规则 → 9 → attempt 2 一条规则 → 7; 每条规则留一消融真阳性全存活 (reviewer 判非对症下药)。**验证 attempt 1**: 召回 1/1 + 预登记外新真阳性 1, 假阳性字面 7 > 5 FAIL (CITATION_ERROR 3 + 纯 FP 4) → 归档 `failures/c2r_n5_gate_attempt_1.md`; **用户裁定 CITATION_ERROR 计入**; **attempt 2** (画面句不归卡片单元, 预登记) 9 → 7, 假阳性 5 ≤ 5 **压线 PASS 零余量**, screen_dropped = 2 (验收面 example 级)。judge 全核 SIBLING 16 条抓 1 漏检 ⇒ 真实召回 2/3; 「关系说反」维零判别力。
...
### 2026-09-15 (下午) — DM2 研读包旁路「整本喂」(默认 ON, 生产已 kickstart; e2e attempt2 6/6, Claude 对比待 Bedrock)

- **起点**: DM1 D4 区分实验裁定 —— study 侧候选卡 0/32 的根因是「SDTM 域定义 ↔ EDC 日文 label」语义鸿沟, 检索层无廉价桥。用户裁定: **不做「域→表单」人手表, 走整本喂**。spec/plan `docs/superpowers/{specs,plans}/2026-09-15-study-dossier*`, SDD ledger `.superpowers/sdd/2026-09-15-study-dossier/progress.md`, subagent 驱动 (T1-T10, 每 task 异 agent review, 规则 D)。
- **落地** (commits `3453652`..`3131ddc`, 17 个): T1 `server/study_dossier.py` 构建器 (PRT s4-s12 白名单章 + EDC 全项目一览, 纯文件读, 超预算抛错); T2 token 硬前置闸 (166563 字 / **133652 tok**, cl100k_base 估计 —— litellm 不认 bedrock id 静默回落, 四档数字逐位相同; 用户裁定**含选择肢**); T3 `dossier_trigger.decide_dossier` 纯函数 (域码 D1 注入 + 范围词类, 收紧去裸 `EDC`/裸 `in our`); T4 配置默认 ON + 启动 fail-loud; T5 `router.maybe_attach_dossier` 共用 helper (丢 study chunks / 强制 both / 规则句 / 徽章字段, OFF 逐字节不变); T6 `retrieval_levers.dossier`; T7 webchat 三态控件 + 📖 徽章 + ⚑ 存档; T8 L2 零 LLM 闸; T9 L3 e2e; T10 收口。
- **闸** (`evidence/checkpoints/dm2_{gates,dossier_tokens,dossier_e2e}.md`): 触发扫描 `cdisc140` **0/140** / `study48` **0/51** / `mapping8` **7/8** (`dm08` 长名前缀不触发 = D1 已知限制, 需手动 on); 检索层逐题 diff 两集 **worse=[] better=[]** (99.17% / 87.5% 逐位不变 —— 研读包不经 `run_eval`)。T8 attempt 1 `cdisc140` 1/140 (q29 `"f<our Trial>"` 跨词边误触), 修法 = 英文范围词加 `\b` (词类级, 不加题面黑名单) + 负例测试。
- **L3 e2e** (3 题 × 2 模型 id, 生产 `/api/ask_stream`): attempt 1 **4/6 FAIL** —— `DSCAT` 第三类被静默漏掉, 用 `DSSCAT` 子类别 / 阶段轴顶替 (**类别轴混淆**, pattern 级, 凡分类轴 >1 的域都会复发)。修法只动 `_DOSSIER_RULES` 规则层 (沿 `--CAT` 轴逐类穷举 + 每类各起小标题 + 「要么列候选要么明写候補なし」+ 逐条 (推測)), **不写任何域名/CT 取值/OID**; 并加测试钉住实际拼进 system 的措辞。attempt 2 `--no-resume` 重跑全 6 题 → **6/6 PASS** (捏造 0/6, 958 项一览逐 token 核过; `prompt_tokens` 三题一致 +83 证新规则上线)。
