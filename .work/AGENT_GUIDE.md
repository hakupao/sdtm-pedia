...
- **"KG 价值 eval 开始任务"** → `branches/07_rag_kg/sdtm-rag/KG_VALUE_EVAL_KICKOFF.md` (SP3 收尾时立项)。**测 SP1-3 的 KG 答题通道是否真让答案变好** (OFF-vs-ON 真模型+judge, 图类问题): SP3 三门是确定性的 (引擎 vs meta.yaml + 140q 零污染), 没测过端到端 LLM 答案质量。盲写图能力题集 → `--graph-answer` OFF/ON 配对 + judge → 判定 KG 价值证实/证伪 (决定 SP4/SP5 值不值得投)。详 kickoff 文档。
