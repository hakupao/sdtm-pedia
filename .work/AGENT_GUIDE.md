...
- **"KG 重启 开始任务"** → `branches/07_rag_kg/sdtm-rag/KG_ROADMAP.md` (SP1-5 子项目路线图, 用户要全做) + memory `project_kg_decision`。**brainstorming 进行中**: SP1-5 拆分已 ack, **恢复 SP1 (meta.yaml) 设计问答** (待问 5 题见 ROADMAP)。⚠️ HARD-GATE: 设计定稿+用户批准前别 writing-plans/写码。能力在 SP1-3 用 meta.yaml+内存图交付, **Neo4j 是可选界面非能力前置**
