...
- **"KG 重启 开始任务"** → `branches/07_rag_kg/sdtm-rag/KG_ROADMAP.md` (SP1-5 子项目路线图) + memory `project_kg_decision`。**SP1 (meta.yaml 元数据层) DONE ✅ 2026-06-17** (纯数据层: `data/meta/meta.yaml` 64 域 + `scripts/{build_meta,reconcile_meta}.py`; reconcile gate 抓修 spec_loader 247 幻变量 bug; Gate1 8/8 + Rule D 异type APPROVE + Rule A N=8 PASS)。**下一个 = SP2** 确定性答题通道 (meta.yaml 载内存 → q103/q104 翻绿 + 退役 structured_lookup 正则); 走同样 brainstorm→spec→plan→impl 流程。能力在 SP1-3 用 meta.yaml+内存图交付, **Neo4j 是可选界面非能力前置**
