...
- **"KG 重启 开始任务"** → `branches/07_rag_kg/sdtm-rag/KG_ROADMAP.md` (SP1-5 子项目路线图) + memory `project_kg_decision`。**SP1 (meta.yaml 元数据层) DONE ✅** + **SP2 (确定性结构化答题通道) DONE ✅ 2026-06-20** (Phase1 答题通道默认 ON, q103/q104 翻绿; Phase2 退役 structured_lookup 正则影子 KG 改读 meta.yaml, 严格行为等价 9891/9891, Rule D APPROVE)。**下一个 = SP3** 关系/影响查询 (meta.yaml 之上内存图遍历 networkx/纯 Python) = **新设计单元, 必须先 `superpowers:brainstorming`** (无现成 spec/plan, HARD-GATE)。能力在 SP1-3 用 meta.yaml+内存图交付, **Neo4j 是可选界面非能力前置**
