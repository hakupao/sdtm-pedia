# P0 Writer MD — md 原子化 prompt v1.2

> Version: v1.2 (2026-04-24, post-P0 Pilot 收官)
> 基于 PLAN v0.4 + P0 Pilot 收官 findings (FINAL_report.md §5 H1' + §6 v1.2 in-flight)
> 角色: Writer (原子化), 独立 subagent, 与 PDF Writer 和 Reviewer 不同 subagent_type
> v1.2 变更 over v1: (a) **H1' fix** — `*.xpt` / `*.sas7bdat` / dataset 文件名硬归 CODE_LITERAL 无论语法上下文; (b) **N1 fix** — 9-type enum validator 硬 gate 拒 `PARAGRAPH` 等造词; (c) 派发 subagent_type 硬约束 executor/writer 家族
...
**禁用**: `Explore` / `oh-my-claudecode:explore` / `feature-dev:code-explorer` 家族 (证据: `failures/v1.1_attempt_pdf_writer_Explore.md`).

**工作模式**: 直接 Write tool 追加 JSONL 到 `output_file`, 最终消息只回 `DONE file=<name>, atoms=<N>, failures=<N>`.
...
## 原子类型 — 9-TYPE ENUM 硬 gate (v1.2 强化, N1 fix)
...
**禁用示例** (v1.1 实测 N1): `PARAGRAPH` / `PARAGRAPH_INVALID_AGENT_ERROR` / `BULLET` / `BLOCKQUOTE` / `SECTION` — 全非 schema 枚举. 写入前 self-check, 发现即重写.

**"多句段落" 处理法 (v1.1 教训)**: md 内 1 段含 N 句 → **拆 N 个 SENTENCE 原子**, 绝不合并成 1 个 `PARAGRAPH` 假原子. 段落本身不是 atom_type.

md 特有形态:
- `HEADING`: md 中 `#`/`##`/`###` 等; `heading_level` = `#` 数量
- `TABLE_HEADER` / `TABLE_ROW`: md `|...|` 表格
- `CODE_LITERAL`: inline `` `C66742` `` 或段落内字符串常量或 dataset 文件名 (见 §v1.2 CODE_LITERAL 硬规则)
...
```jsonc
{
  ...
  "figure_ref": null,
  "cross_refs": ["§X.Y", ...],
  "extracted_by": { "subagent_type":"...", "prompt_version":"P0_writer_md_v1.2", "ts":"..." }
}
```
...
3. **verbatim 保原 markdown**: 含 `**bold**`, `*italic*`, `` `code` ``, `[link](url)` 语法
...
6. **CODE_LITERAL 硬规则 (v1.2 H1' 扩)**:
   ...
   - Dataset 文件名 `*.xpt` / `*.sas7bdat` / `*.csv` (如 `cm.xpt`, `ae.sas7bdat`, `dm.xpt`): **必 CODE_LITERAL 无论语法上下文**. 即便后有冒号 (`cm.xpt:`) 或作名词短语一部分 (`the cm.xpt dataset`), 文件名本身永远是 CODE_LITERAL 原子.
     - T2 实测教训 (FINAL_report §5 H1'): `cm.xpt` 被 MD writer 加冒号后归 NOTE, matcher 宽判 EQUIVALENT 0.92 掩盖 — 禁止复发.
   - Codelist 字面值 (`"NOT APPLICABLE"`, `"Y"`, `"N"`): CODE_LITERAL
   - CODE_LITERAL 与外层 SENTENCE 并存
7. **line_start / line_end 必精确**: 用于 P6 修复时的行号偏移检测 (R11)
...
5. 检查 `*.xpt` / `*.sas7bdat` / `*.csv` 模式在 verbatim 中出现 → 若原 atom_type 非 CODE_LITERAL → 拆出子 CODE_LITERAL 原子
