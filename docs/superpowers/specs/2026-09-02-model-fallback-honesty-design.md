...
⚠ **P3 的边界必须写明**: 它证的是「**开流阶段**失败 → Router 换 deployment → 后续 chunk 来自
fallback」。**流开成功之后中途断掉不会触发 fallback** (`CustomStreamWrapper` 已经返回给调用方了),
那种情况仍旧走 `event: error`。真实世界最常见的失败 (credits 耗尽 / 认证失败 / 限流) 都发生在开流阶段,
所以这条容灾有实际价值 —— 但**不能宣称"流式全程有容灾"**。
...
新函数 `server/llm_config.py :: fell_back(s, model_group, reported_model) -> bool | None`。
...
| `reported_model` 为 `None`/空 | `None` | 同上, 不知道就说不知道 |
| `reported` 与该组配置串**同一个模型** | `False` | |
| 其余 | `True` | |
...
`done` 事件加字段: `"fell_back": <true|false|null>`。回退为真时同时打一条
`log.warning("model_fell_back", model_id=..., model_used=...)` —— 让运维日志里也有痕迹,
不只在用户屏幕上。
...
| 处 | 改动 |
|---|---|
| `onDone` | 多读 `data.model_used` / `data.fell_back`, 一律 `?? null` (三态语义, 与现有 `verified` 同款) |
| `persist` | 存档对象多存 `modelUsed` / `fellBack` |
...
|---|---|---|
| `true` | `模型: {label} → 实际 {modelUsed}（已回退）· 验证状态未知` | **是** |
...
**⚑ 归因** (`flagModelName`): 回退时返回 `` `${modelUsed}（回退自 ${label}）` ``。
...
| G6 | **真 `Router` 端到端**: 主模型开流抛错 ⇒ 流仍成功, `done.model_used` 来自 fallback, `fell_back` 为 `true` | 反: 抽掉该组的 fallback 条目 ⇒ 变成 `event: error` |
| G7 | `fell_back` 三态: 配置串 ⇒ `False`; 别的串 ⇒ `True`; 内部组 / `reported` 缺失 ⇒ `None` (⛔ 不是 `False`) | 三向各一条 |
| G8 | 匹配必须带 `/` 边界 | `reported="claude-opus-5"` (真子串但非完整尾段) ⇒ `True`; 变异成裸 `in` ⇒ 红 |
| G9 | 徽章: `fellBack=true` ⇒ 文案含 `→ 实际 {modelUsed}`+`已回退`+`验证状态未知` 且带 `.unverified` | 反: `fellBack=false` ⇒ 与今天**逐字相同** |
| G10 | 兼容 B1/B2: `fell_back`/`modelUsed` 缺失 ⇒ 徽章与今天**逐字相同** | 正 |
| G11 | 存档诚实: 走完整流 (`onDone`→`persist`) 后 localStorage 里有 `modelUsed`/`fellBack`, 重建 DOM 后徽章仍显示回退 | 反: 不回退的流存档里 `fellBack` 为 `false` |
| G12 | ⚑ 归因: 回退时 `/api/flag` 的 `model` = 实际模型 (**诱饵**: `msgObj.modelId` 是 `gpt-sol`, 读它就红) | 反: 既有两条 (`withModelId` / `legacyNoModelId`) 必须仍绿 |
| G13 | 静态形状闸 (node 缺席时仍在): `.fellBack` / `.modelUsed` 属性读取存在 | 照 `test_flag_payload_reads_the_message_model_id` 双闸写法 |
...
| **L2** | fallback 只覆盖**开流阶段**失败 (§3 P3 边界)。流开成功后中途断仍走 `event: error` |
