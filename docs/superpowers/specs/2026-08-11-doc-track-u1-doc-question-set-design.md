...
单 gold 题附一条 `anchor` (**≥ 20 字符**, 防短串碰巧命中); **多 gold 题
(`len(expected_sources) > 1`) 必须改用 `anchors`** —— list, 与 `expected_sources`
**等长且一一对应**。`anchor` 与 `anchors` **两键互斥**, 同时给出即报。

判定: **`anchors[i]` 必须落在 `expected_sources[i]` 经 `match_names` 解析出的每个 chunk 里,
且任何一条 anchor 都不得落到 gold 集合之外。** 无 gold 直接报; 缺 anchor 报; 长度下限对
**每一条** anchor 生效。OR 组不支持 `anchors`。

> **为什么多 gold 不能用单条共享锚串** (2026-08-12 实测, Task 6 开工前): 单条锚串要求它逐字
> 出现在**每个** gold 里。实测 114 个 chunk 的 20 字 shingle —— 恰好只出现在 2 个 chunk 的共
> 2,715 条, 涉及 **90/6,441 = 1.4%** 的 chunk 对; `s22_1` 切点两侧 **0 条**。
> **98.6% 的 chunk 对不存在这种串, 而这不是缺陷**: 互补的两节本就不共享逐字长串, 共享长串的
> 恰是重复节与父子节。**单条共享锚串证明的是"有句话同时出现在两块" —— 那是重复的证据,
> 而跨节聚合题要的是互补。方向是反的。**
> 旧口径下 §5 点名必做的 `22.1` L6 探针**结构上写不出来**; 逐 gold 锚串把它救回来了。
>
> (口径注: 比对时**必须规范化空白**。不规范化会把 `-layout` 的空格填充算成共享串 ——
> 按原始正文测 `22.1` 得"共享 77 条", 分类后 **52 条全是过半空白的版面残留, 实质文本 0 条**。)
...
- **gold 与答案证据指向同一批 chunk** (而不是"我以为它们指向同一批")
- **答案证据没有溢出到 gold 之外** —— 溢出即说明该题另有 chunk 也能答, 当场红

⚠ 本条初版写的是"题太泛 ⇒ 出现次数 > gold 数, 当场红"。**计数口径已于 2026-08-11 作废**
(见下方两处口径修订), 现行判据是 membership + 逐 gold 对应, 不再比任何计数。
