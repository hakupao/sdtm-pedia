...
> 改为 membership 校验后, **在闸 A 已保证每条 gold 唯一定位的前提下**严格强于原口径。
> gold → chunk 的解析必须复用 `lint_gold.match_names` (与闸 A 同一语义, 不许第二份)。
>
> ⚠ **"严格强于"不是无条件的** (复审实测指出, 2026-08-11): 旧口径比的是 `len(gold 列表)`,
> 新口径比的是解析出的 chunk **集合**。若一题的两条 gold 指向**同一个** chunk
> (重复写, 或 `['s1_1', 's1_1.md']` 这种同指), 旧实现因 `hits(1) != n_gold(2)` 会偶然报,
> 新实现去重后判绿。该题本身仍良构 (锚串确实落在 gold 里), 不制造坏题入池 —— 但
> **无条件写"严格强于"是错的**, 那种句子会被后人引用来跳过复验。
>
> ⚠ **多成员 OR 题**: 新口径要求锚串逐字出现在**每个** OR 成员里, 措辞不同的合法 OR 题
> 会被报 missing (旧口径同样报, 非本次引入)。本 spec 已规定 doc 侧一题都不用 OR 组;
> 若将来要用, 锚串必须逐字出现在每个成员里。
