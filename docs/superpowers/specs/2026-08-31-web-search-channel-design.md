...
| B2 | 逐字节回滚闸测的是「ON 挖掉 Rule 9 == OFF」的内部一致性, 不是 spec 要的「OFF == 引入本功能前」 | 恒定 | 后者在 Task 3 期由 reviewer 与控制器**各自独立**用 `850fd13^` 建 golden 比 sha 验过 (两侧均 True), 但**没有常驻断言**。**✅ 已处理 (2026-09-01)**: **未**采纳 sha256 golden —— 该闸**活不过第一次合法变红**(基础 prompt 因无关理由正当改动时它就红, 而更新它的那一刻它断言的含义就没了; 不是有人偷懒才失效, 是**按正确流程也必然失效**。变异实测: 改一句与联网无关的 rule 3 措辞, golden 变红而 14 条不变式闸全绿)。<br>改钉**长期有效的不变式**: 「联网特性对 prompt 的全部贡献 = web 分支插入的那一段, 且别处不留残留」。顺带补掉逐字节回滚闸此前**未记的两个盲区** —— 措辞**抄进**基础规则(落在挖除区间外)、web 分支**偷塞**一段(落在区间内被一起挖掉), 二者恰好覆盖「往 prompt 加东西」的两种落点, **三条合起来才是完整红线闸**。<br>⚠ **诚实边界 (三条)**: (i) 新闸**证明不了**今天的 OFF 逐字节等于 `850fd13^` —— 那是历史命题, 只有 Task 3 那次一次性核验背书; (ii) marker 列表是**人工挑的** (机械 n-gram 在此会假阳: 9(b) 与 rule 7 有合法共用措辞); (iii) **它不钉 Rule 9 的内容** —— 改写一句非 marker 措辞 1899 条全绿 (闸的两边同源, 属 retrospective 规则 6 成因 **B2**)。命题是**结构**不是内容, 别据此以为内容受保护。 |
| B4 | `web_search.py::_bump_day()` 对**类级** `_day_used` 做 read-modify-write (`+= 1` 是 LOAD/OP/STORE 三步) 且 `if cls._day != today` 是 check-then-act | 并发请求 + `asyncio.to_thread` (Task 4 F-1 修复后引入) | Task 4 前所有搜索串在唯一 event loop 线程上**天然互斥**, 修复后并发跑进线程池。reviewer 实测 24 线程 × 5000 次 × 3 轮**零丢失** (GIL 版 CPython 3.14.4)。⇒ 当时判 Minor 记账 (影响面仅日配额**软闸**计数精度, 不碰 `web_max_searches` 每请求**硬闸**)。**✅ 已修 (合并前终审)**: `_bump_day()` 现由类级 `threading.Lock` 保护, 日期翻转的 check-then-act 与 `+= 1` 同处一个临界区。终审驳回了原来的记账理由 —— 压测零丢失证明的是**窗口窄, 不是操作原子**, 而「换 free-threaded 构建时记得加锁」这类注释的历史命中率接近零, 故不留 TODO 直接上锁。⚠ **「已上锁」≠「日配额跨进程强一致」**: `_day_used` 是**类级**变量, `threading.Lock` 只保护单进程内的并发; uvicorn 多 worker 下仍是每 worker 各算一份日配额 —— 这是 B4 原本就有的性质 (代码注释写着「非跨进程强一致」), 本轮**没有改变**它。真正的每请求**硬闸**是 `web_max_searches`, 日配额始终只是**软闸兜底** |
| B3' ⛔ | **eval 已无法复现生产 system prompt**（终审 I-C，**升级自 B3**）| 恒定，**今天就在咬** | 生产每次请求（含 `web=false`）都带 Rule 9；eval 写死 `web_search_enabled=False` 且无 `--web-search` lever。终审变异实测：两处 `False` 改 `True`，**1862 条全绿**，值漂无人响。⇒ **现有 140q / study 48q 数字描述的是一个生产不跑的构型**。<br>⛔ **硬约束（2026-09-01 重划作用域 —— 原文按「题集」写，写宽了）**<br>**⊘ 不在本条范围内：`--retrieval-only` 的 source recall 数字。** `eval/run_eval.py` 模块 docstring 自证该模式 "no LLM calls"，`run_evaluation()` 内 `if not retrieval_only and (...)` 跳过答题调用；而两个 lever 只往 **answer 侧 system prompt** 追加文本 (`server/rag.py:199-204`)。⇒ 该 prompt 增量**从未被消费**，lever 取值在因果上不可分辨。**实查：PROGRESS 门面 7 数（CDISC 99.17%/98.93%/81.07%、study 87.50%/80.49%/100.00%、U1 88.33%）全部是 retrieval-only**，各有命令行出处。说它们「描述生产不跑的构型」＝说一把不量温度的尺子读错了温度。<br>**✅ 在本条范围内：带答题模型的 run。** 对这些 B3' 完全成立，且**比原文更重** —— 它们不只缺 Rule 9，其中多条还是 `prompt_guardrail=False`（落盘实值），即比生产少了**两块** prompt，原文只点了 Rule 9 一块。<br>⛔ **约束指向实际引用点**（原文的 ⛔ 悬着，没指向任何具体位置）：<br>　· `README.md:346` / `README_CN.md:346` 「53-question eval 88.5% PASS」—— **答题侧**，且跑在 guardrail 与 Rule 9 **都还不存在**的时候。<br>　· `milestones/07_rag_kg/RETROSPECTIVE_retrieval_arc.md:19` 「端到端 fact recall **93.9%**」—— **答题侧**，源 run 落盘 `prompt_guardrail=False`，而生产默认 `True`（`server/config.py:118`）。<br>**2026-09-01 进度**：lever 已补（见下方 B3 行），但 `--web-search` 默认 off ⇒ **对答题侧数字，本条未解除**。解除条件：**以 `--web-search`（+ `--guardrail`）真跑一遍答题侧**。别把「flag 已加」读成「B3' 已修完」。<br>⚠ **记账陷阱**：retrieval-only 照样构造 RAGEngine、照样在回执打 `guardrail=ON`、照样往 summary 落 `prompt_guardrail`/`web_search`⇒ 修复前**一份 retrieval-only 产物上的 `guardrail=ON` 是无因果意义的标签**，不可反向读成「该数字受护栏影响」。**已于 2026-09-01 修复**：现由 `print_summary()` 落的 `retrieval_only` + `n_answered` 两键作证。见 B6 行。<br>⚠ **一条未坐实**：`judge_input_v3.json` 取自 `v3_full_on_t0`(guardrail OFF) 还是 `v3_full_on_guardrail_t0`(ON) —— 按 substring 对照值 82.6% **推断**为前者，但两份 JSON 已从工作树删除，**这是推断不是取证**。要坐实需比对 judge 输入与两份 run 的 answer 文本。<br>修法：加 `--web-search`，用同一 `args` 同时驱动工具循环与本 lever；不必专门花钱重跑，等下次因别的原因跑 eval 时顺带跑一遍即可。<br>⚠ 风险形状**不是**「Rule 9 把答案搞坏」（抽检 Q3 已证否，见 §10.2），而是「Rule 9(b) 可能让模型在涉码题上**系统性变保守**」——渐进、静默、朝向"更安全"一侧，**人工抽检天然不敏感**，只有 140q + `check_code_grounding.py` 能量化。 |
| B3 | `eval/run_eval.py` 的 `web_search_enabled` 恒 `False` (兄弟 lever 走 `args.guardrail`, 唯独它写死) | 未来加联网评测时 | **✅ lever 已补 (2026-09-01, 分支 `fix/eval-web-search-lever`)**: `--web-search` (`store_true`), 两处注入点 (cdisc 主引擎 + 联邦 `study_levers`) 共用同一个 `args.web_search`, 屏幕回执 + summary JSON 均落盘, 9 条新闸钉住两个方向 × 两个注入点。<br>⚠ **未来加工具循环时必须用这同一个 `args.web_search` 同时驱动循环与本 lever**, 不得新开第二个 flag, 否则会跑「无 Rule 9 构型」却当生产数字上报 (代码注释已延续本条)。<br>⚠ **旧注释是假约束, 已删**: `study_levers` 那处原写「恒 False 是为保 `docs_engine_parity` 闸不漂移」—— 但那道闸断的是**键集** (`set(evl) - set(prod) == EVAL_ONLY_LEVERS`), `web_search_enabled` 两边都传, 取值换成什么对它毫无影响 (实测照绿)。**给出一个不存在的约束当理由, 比过期注释更害人** —— 它会让后来人以为动这里会打破 parity 闸而不敢动。(实现者提出, 控制器独立核实) |
| B5 | **兄弟 lever `prompt_guardrail` 同款洞** —— `guardrail_info` 与 `summary["prompt_guardrail"]` 都读 `args`(意图) 而非引擎实收值(事实), 且注入点无取值断言 | 恒定, 发现于 2026-09-01 复核轮 | **✅ 已修 (同分支)**。发现路径值得记: 实现者主动报了一条「记账」(同一 dict 里 `web_search` 记事实 / `prompt_guardrail` 记意图), 复审顺着那条注释做变异, 挖出这个洞。<br>**变异实测 (修前/修后, 每格跑全量)**: cdisc 钉 `True` 1874 全绿 → **4 failed**; cdisc 钉 `False` 1874 全绿 → **4 failed**; `study_levers` 钉 `True` 1874 全绿 → **1 failed**; `study_levers` 钉 `False` 1 failed(parity) → **2 failed**。⇒ 修前**四格里三格全裸** (实现者订正复审: 复审只测了 `False` 方向, `True` 方向同样全裸 —— parity 闸的 `docs["prompt_guardrail_enabled"] is True` 只在传了 `--guardrail` 时才断言)。<br>⛔ **修前可以跑出标着 `guardrail=ON`、实际全程 OFF 的 140q 数字, 1874 条无一响。** 凡 2026-09-01 之前产出的 guardrail=ON 数字, 其构型未经机器验证。<br>**教训**: 一类缺陷极少只出现一处 —— B3' 修完立刻查兄弟 lever, 隔 8 行就有一个。 |
| B6 | **retrieval-only 产物上的答题侧 lever 是无因果意义的标签, 且产物无法自证** | `--retrieval-only` 的每一次跑批 (= 本仓库门面 7 数的全部来源) | **✅ 已修 (2026-09-01, 同分支)**。`--retrieval-only` 不发任何 LLM 调用 (`eval/run_eval.py` 模块 docstring 自证 "no LLM calls" + `run_evaluation()` 内 `if not retrieval_only and (...)` 跳过答题), 故 `prompt_guardrail` / `web_search` 两个 lever **完全空转**; 修前回执照打 `guardrail=ON`、summary 照落两个 lever, 而 `retrieval_only` 键**不在 summary 里**。<br>**已实施**: `print_summary()` 落**两个**键 —— `retrieval_only`(模式) + **`n_answered`**(事实); 回执用常量 `INERT_LEVER_MARK` 给开着的 lever 各加 `[INERT: retrieval-only, no LLM call]`。两个 lever 的**键与类型未动**(仍 bool), 既有消费方不受影响 (全仓无 summary 键集/schema 校验, 已查)。<br>⚠ **实现者两处订正控制器, 复审独立裁定均成立**: (1) 原文「**无法判断**」过强 —— 今天可经 `set(full)-set(src)` 五键差集反推, 但那是**未声明、无人守护、纯偶然**的不变量 (给 `overall` 加默认值即当场失效且无人变红) ⇒ 成因不是「信息不存在」而是「**信息只以口口相传的形式存在**」。(2) **只落 `retrieval_only` 不够 —— 它是意图不是事实**: 答题分支还带 `router is not None or direct_model is not None`, 故 `retrieval_only == False` **不蕴含真的答了题**。变异实测: 把该 `or` 写成 `and`, 屏幕打 `[FULL] src=100% fact=0%`、产物 `retrieval_only: false` + `prompt_guardrail: true` 而**一题没答**, 只钉 `retrieval_only` 的闸全绿; 加 `n_answered` 后 1899 条里正是它变红。<br>**归类**: retrospective 规则 6 成因 **B1** 的变体 —— 键在、值也对, 缺的是**这个值有没有生效**那一维。 |
| B7 | **`run_evaluation()` 重试耗尽后跨题串答案** (既有缺陷, 非本分支引入) | 某题 5 次重试全失败 | ⛔ **未修**。`response` 是**函数级**变量、**跨题持久**: 第 1 题耗尽 → `NameError` (吵闹, 能发现); 但**第 N>1 题**耗尽时 `response` 仍持有**上一题**的响应 ⇒ 把上一题的 answer / usage / model **静默记到本题上**, 并照常算 fact_recall。<br>⛔ **长跑批里更可能发生的正是后者, 产出一份看不出异常的错数据。** (实现者报的是 NameError 那一半; 复审指出跨题串用这一半更重, 按此形状记账。) |
...
**体量的两句话必须同时说**（缺一条都是误导）：Rule 9 使 system prompt 从 3053 -> **4686** 字符，增量 **1633 字符 ≈ 408 token**，**占指令层 34.9%**（作对照，`_GUARDRAIL_RULES` 整块才 1946 字符 —— 即 Rule 9 已比整块护栏还长）；⚠ **数字更新于 2026-09-01**：原记 `4363 / 1310 / 327 token / 30%` 已过期 —— 终审把反注入句移进引言段（`941864f`）后 Rule 9 自己长了 ~323 字符。实现者与控制器**各自独立**复算，增量一致（绝对值随 `_routing_md`/`_index_md` stub 长度浮动，**增量不受影响**，故以增量为准）；
