...
### 1.4 ⛔ ~~基线证伪了 §1.1 的根因假设~~ —— **本节标题与结论已于 2026-08-13 撤回**

> **撤回说明(controller 自陈)**:本节初版标题为「基线证伪了 §1.1 的根因假设」,
> 该结论**不成立**,由 Task 6 独立审阅推翻、controller 自行复算确认。
> **错误影响了一次用户裁定**(基于错误依据选择了「只改规则 3」),已重新裁定。
> 下方数字表**全部属实、未改动**;被撤回的是从数字推出的**结论**。
> 正确结论见 §1.4.1。**保留原文而非删除**,因为「怎么错的」比「结论是什么」更值得留。

**原推理(错误,留档)**:手順書型题 51 道(u1_doc 27 + dev 12 + heldout 12)判对 50 道
⇒ 若规则 2 真够不着手順書,这 51 道该大面积错 ⇒ 根因假设被证伪。

**两根支柱各自为什么塌**:

1. **「u1_doc 27/27」是循环论证。** `eval/run_routing_eval.py` 的 `FINAL_IDS` 把 U2 已实测
   判错的那三题切出去,**剩下的 27 题按构造就是「已知会对的题」**。这个 27/27 在 U2 那次
   测量结束的瞬间已经恒真,**本次跑批没为它增加一比特信息**,却被写进了分母 51。
2. **「新写 24 题只错 1 题」没有统计功效。** 唯一真正新增的证据是 23/24。
   Fisher exact 对 U1 的 3/30:**双侧 p = 0.6204**;若真实失败率就是 10%,
   24 题中出 ≤1 例的概率 **0.2925**。两组统计上无法区分。
   (controller 独立复算,数字与审阅方一致。)
3. **形态欠采样(更要命)**:四道 `study → cdisc` 失败题(3 道豁免 + `u3_doc_02`)形态一致 ——
   **纯医学域的分類体系 / 定義 / 判定基準**,且全部落在规则 1 排除条款自己列的例子射程内
   (`adverse events` / severity **grading** / `lab results` / `dosing`)。
   而新写 24 题**以運用実務型为主**(規定値 / 期限 / 区分界線 / 手続 / 機関要件 —— 出题方
   task-4-report §2 自己的设计声明)。**落在失效形态里的新题实测只有 `u3_doc_02` 一道,它错了。**
   最接近的两道(`u3_doc_08` 術後早期/晩期の日数境界、`u3_doc_16` 拒否/撤回の区分)是
   **研究程序性**区分而非医学域概念,判对了。
   ⇒ 「新写的 24 题恰好都容易」这个替代解释**不但排除不掉,还有出题方自己的设计声明作证**。

### 1.4.1 ✅ 基线的正确读法(修订后)

**gold 在测量前已冻结**(防线 1 成立),基线三遍完全一致:

| 组 | 基线 | 说明 |
|---|---|---|
| legacy | 179/181 | 与历史一致 |
| u1_doc | 27/27 | ⚠ **循环,零信息**(见 §1.4 第 1 条) |
| dev | 12/12 | 多数类基线 100%,见 §7.1 |
| heldout | 11/12 | 仅 `u3_doc_02` 错 |
| final(豁免) | 0/3 | 三题全 `study → cdisc` |
| distractor_cdisc | 7/12 | 3 题 `→study` 属 fatal / 2 题 `→both` 非 fatal |
| **ambiguous_both** | **0/6** | 6/6 整齐判成 `study` |

**结论一(§1.1 得到加强而非推翻)**:在 U1 那 30 题上基线是 **27/30,三题全部
`study → cdisc`,与 §1.1 完全同向 —— 本次基线一比一复现了 U2**。
新题又贡献了 **第 4 个实例** `u3_doc_02`(同形态,且与 `q17`/`q53` 同属第 3 章),
且它是**盲写**的(出题方未接触路由器、未被告知失效形态)⇒ **证据强度高于前三例。**

**结论二(两个失败方向并存)**:10 个 fatal(排除 final)分属两侧 ——
`ambiguous_both` 6 题 `both→study` + `distractor_cdisc` 3 题 `cdisc→study` 为一侧,
`heldout` 1 题 `study→cdisc` 为另一侧。
⚠ **不得**把前者概括成「路由器过度判 study」——`distractor_cdisc` 与 `ambiguous_both`
**都是对抗构造题**(前者刻意做脆、后者专补 `ja_supplement` 自认的空白),
**其错误率不能外推为路由器的总体倾向**。(此处 controller 初版亦犯了外推错误,一并撤回。)

**结论三(另一处被自身数据证伪的表述,留档)**:「路由器根本不产出 `both`」**不成立** ——
全 253 题 `pred=both` 共 **9 次**,且 legacy 的 5 道 `gold=both` **5/5 全对**。
真实形状是「**会给 `both`,但给错了地方**」。

### 1.4.2 ~~原 §1.4 数字表~~(内容已并入 §1.4.1,以下保留原始记录)
...
~~⇒ 手順書型题 51 道判对 50 道。§1.1 被证伪。~~ **← 已撤回,见 §1.4 撤回说明与 §1.4.1**
~~⇒ 9/10 = 过度判 study;§6.1 原定修法方向错误。~~ **← 外推错误,已撤回**

(以上两条结论为 controller 初版所写,均已撤回;数字本身属实。)

### 1.5 目标重定向(用户裁定 2026-08-13,**已按 §1.4.1 修订后重新裁定**)

> ⚠ 本节初版为「只改规则 3」,那次裁定**建立在 §1.4 已撤回的错误结论上**。
> 用户在收到更正后**重新裁定:两个方向都改**。留档如此。

**修法 = 同时动两处:规则 1 的排除条款 + 规则 3 的 `both` 判据。**

理由:**条款 1 要求 fatal=0,而 10 个 fatal 分属两个相反方向,只修任一侧都过不了**。

| 方向 | fatal | 修法 | 依据 |
|---|---|---|---|
| `study → cdisc` | 1(`u3_doc_02`)+ 3 道豁免 | **规则 1 排除条款** | §1.4.1 结论一:4 个实例同形态,第 4 个是盲写新题 |
| `both → study` | 6(`ambiguous_both`) | **规则 3 `both` 判据** | 补 `ja_supplement` 自认空白;gold 测量前已冻结 |
| `cdisc → study` | 3(`distractor_cdisc`) | 不直接改,作**过宽惩罚器** | 对抗构造题,刻意做脆 |

⚠ **两个方向互相拉扯,这才是本单元真正的工程难点**:放宽规则 1 的排除条款(让纯医学域的
分類/定義/判定基準归 study)会**同时**提高那 3 道刻意做脆的干扰题被误判为 study 的风险。
**条款 4(干扰题下降 ≤ 1 题)就是这条张力的闸。**

⛔ **对症下药风险与防线(两个方向各一条)**:

| 方向 | 失败面 | 实现方可看 | 实现方不可看 |
|---|---|---|---|
| 规则 1 | 3 道豁免 + `u3_doc_02` | **一道都不可看** | `final` 与 `heldout` 全部题面 |
| 规则 3 | 6 道 `ambiguous_both` | `ja_supplement` 5 道**带标记**的 `both` 正例(git tracked,基线 5/5 全对) | 那 6 道的题面 |

⇒ **两侧的失败样本全部是 held-out。** 规则必须从**概念**写出来,不是从例子拟合出来:
- 规则 1 侧的概念:「**本研究自己规定的**分類体系 / 判定基準 / 定義,即使其主题是通用医学概念」
- 规则 3 侧的概念:「**无标准侧显式标记**、只凭主题就同时需要两库」

⚠ **重新划分任何一组 = 看过数据后动尺子,禁止。**
...
### 6.1.1 改动面扩大(用户裁定 2026-08-14,Task 7 第 5 轮)

Task 7 前 4 轮实测:两方向条款(规则 1 排除条款 + 规则 3 `both` 判据)纯追加后,
可见集 191/193 → **185/193**(legacy exact **173 < floor 178**,条款 1 确定性 FAIL);
消融显示两条款各 +2、叠加 **+6(超加性)**,单留任一侧 177 仍破 floor ⇒ 无部分保留方案。
实现方机理判断:末尾既有兜底句 "answer both in every remaining case" 把两条款给出的
「另一侧也有事实」放大成 `both`。

**裁定**:第 5 轮授权把该兜底句纳入重写(连同两条款一并调整)。
约束:兜底句「宁可多查不可漏查」的安全意图必须保留 —— 重写**不得**指示模型在真不确定时
去猜单库;要收紧的只是「有一条规则明确覆盖时仍被拽去 both」的形态。
其余纪律(pattern 不 example / 词表闸 / 规则 2 一字不动 / 只看 dev + 可见集)不变。

**事前登记(先于结果)**:第 5 轮可见集仍 legacy < 178 或 dev < 10/12 ⇒
Task 7 全部退回(revert),记 FAIL 归档,不再有第 6 轮。**本条写在看到第 5 轮结果之前。**
