...
### 1.4 ⛔ 基线证伪了 §1.1 的根因假设(T3 实测,2026-08-13)

**gold 在测量前已冻结**(防线 1 成立),基线三遍完全一致。实测:

| 组 | 基线 | 说明 |
|---|---|---|
| legacy | 179/181 | 与历史一致 |
| u1_doc | **27/27** | U1 其余 27 道手順書题**全对** |
| dev | **12/12** | 新写手順書题**全对** |
| heldout | **11/12** | 仅 `u3_doc_02` 一题错 |
| final(豁免) | 0/3 | `study → cdisc`,与 §1.1 假设同向,但 n=3 且条款 5 不作判据 |
| distractor_cdisc | 7/12 | 5 题错(3 题 `→study` 属 fatal / 2 题 `→both` 非 fatal) |
| **ambiguous_both** | **0/6** | **6/6 整齐判成 `study`** |

⇒ **手順書型题 51 道判对 50 道。** §1.1 写的「规则 2 没有通向手順書的路」**被证伪** ——
若真够不着,这 51 道该大面积错。

**10 个 fatal(排除 final 组)的方向是分裂的,且与 §1.1 相反**:

- `ambiguous_both` 6 题 `both → study`  ┐
- `distractor_cdisc` 3 题 `cdisc → study` ┘ **9/10 = 过度判 study**
- `heldout` 1 题(`u3_doc_02`)`study → cdisc` —— 仅此 1 题与 §1.1 同向

⇒ **§6.1 原定的修法(放宽规则 2 使其更易判 study)方向错误** ——
照做会让 9/10 的 fatal 更糟,条款 1 与条款 4 双双恶化。

**另一处被自身数据证伪的表述**(实测方自查,留档不掩盖):
「路由器根本不产出 `both`」**不成立** —— 全 253 题 `pred=both` 共 **9 次**,
且 legacy 的 5 道 `gold=both` **5/5 全对**。真实形状是「**会给 `both`,但给错了地方**」。
两种说法指向的病因不同(「教它输出 both」vs「纠正它对这类题的判据」),按错的那句改会走偏。

### 1.5 目标重定向(用户裁定 2026-08-13)与新的 held-out 设计

**修法改为动规则 3(`both` 的判据),不动规则 2。**

正当性(不是「看了数据改目标」):
1. 那 6 道 `ambiguous_both` 的 **gold 在测量前已冻结**,不是看了结果才定的;
2. **条款 1(fatal=0)本来就把它们变成必过项** —— 10 个 fatal 里 6 个在这;
3. 它补的正是 `ja_supplement` 文件头**自认的空白**(「無標準側標記の真両可形態未覆盖」)。

⛔ **新的对症下药风险与其防线**:失败面(6 道 `ambiguous_both`)**不属 dev/heldout**。
若让实现方看那 6 道,等于在全部样本上直接调。**重新划分它们 = 看过数据后动尺子,禁止。**

**故 Task 7 的实现方**:
- **可以看**:`eval/routing_gold_ja_supplement.yml`(git tracked)里 5 道 `gold=both` 的题
  —— 它们基线 **5/5 全对**,是**带标准侧显式标记**的那一类,作正例;
- **不许看**:`routing_gold_docs.yml` 里任何 `group=ambiguous_both` 的题面,
  也不许看 `distractor_cdisc` / `heldout` / `final` 的题面;
- ⇒ **全部 6 道 `ambiguous_both` 对本次修法是 held-out**,规则必须从**概念**写出来
  (「无标准侧显式标记、只凭主题就该判 both」),不是从例子拟合出来。

### 1.6 条款 1 的事前登记(先于结果,2026-08-13)

若修法后**唯一剩下的非 final fatal 是 `u3_doc_02`**(held-out、方向与本次修法相反):
**仍按条款 1 触发处理并上报用户**,不得自行豁免、不得回看 held-out 调 prompt。
理由:条款 5 与 U1/U2 的先例一致 —— 豁免是用户的裁定,不是执行方的。
**本条写在看到结果之前。**
...
| 新写 手順書型 | 24 | study | 114 chunk 按 **20 章**分层抽,与 U1 30 题**不重叠** |
...
### 5.2.1 「答得了」的口径(执行期裁定,2026-08-13)

Task 4 审阅交上来三道 gold 有争议的题,争议点归结为同一个问题:
**「读者能从这个语料**推出**答案」算不算「这个语料**答得了**」?**

**裁定:不算。** 口径写死为 ——

> **「答得了」= 语料里有能直接支撑该断言的文字,不含读者自行推演的步骤。**

理由:判库决定的是**去取哪个库**;一个只够让人推出结论、并不陈述该事实的语料,
支撑不了有据可依的回答。这与本仓 `server/federation.py:_FEDERATION_RULES` 已有的纪律一致
(「任何映射都是推测,必须显式标注 (推測/inference),不得当作有据事实呈现」)。

按此口径,三道争议题**全部维持原 gold**(`u3_amb_01` = `both` / `u3_dist_11` = `cdisc` /
`u3_amb_02` = `both`)。**后续出题复用本口径。**
...
4. **dev / held-out 的划分由出题方之外的人做**,且用**确定性规则**而非挑选
   ...
   且跨章分布对称;「按章号奇偶」做不到这一点 —— 实测 24 题分布在 **19 个章**,
   奇数章 9 题 / 偶数章 15 题,按章号奇偶切只能得到 9/15。
...
写成 **pattern 不写成 example**: 规则必须描述**问题的对象**(本研究自己的决定 vs 标准的表示),
**不得举任何具体临床概念作例子** —— 疾病名 / 解剖名 / 検査名 / 薬剤名 / 具体数値閾値 / 域码 /
变量名 / 特定評価体系名,一类都不许出现。(逐类平铺是有意的:**只点其中几类等于给方向**,
本单元已因此在 plan 里栽过一次,见 §5.3 第 1 条的同类教训。)
机械闸 = 词表比对,词表由 controller 预置在 gitignored 文件里,**实现方不需要也不许打开它**。
...
### 7.1 ⚠ 条款 2 与条款 3 的多数类基线是 **100%**(执行期发现,2026-08-13)

Task 4 审阅指出新写 42 题的多数类基线是 57.1%;**往下推一层更严重** ——
`dev` 与 `heldout` 各 12 题**全部 `gold=study`**(它们就是那 24 道手順書型题的两半)。
⇒ 一个「**一律答 study**」的退化规则在两组上都拿 **12/12**:

- 条款 3(dev ≥ 10/12)—— 平凡满足
- 条款 2(held-out ≥ dev − 25pt)—— 平凡满足(两边都 100%,gap = 0)

**这两条判据单独看零判别力。** 真正拦住退化规则的是:

- **条款 1** —— 旧 181 子集里有 **151 道 `gold=cdisc`**,退化规则会让 exact 崩到 ~30/181
- **条款 4** —— 12 道新写 cdisc 干扰题会全错,远超「下降 ≤ 1 题」

⇒ **闸作为系统成立,但六条里有两条自己不携带信息。**
这是 U2 §4.2 那个母题的复发:**判据没被证明能区分它声称要区分的两种情况。**

**处置(加严,不放宽;阈值一字未改)**:

1. **条款 2/3 的数字不得单独引用。** 引用时必须同时给出条款 1 与条款 4 的结果;
   任何只报「dev 11/12」而不报另外两条的写法,等同于报了一个多数类基线。
2. **Task 8 的判定脚本必须把多数类基线并排打出**(dev / heldout 各 100%,
   新写 42 题 57.1%,全 253 题按 gold 分布计算),让读者一眼看到余量。
3. 收口证据的「不能证明什么」必须含这一条。

**为什么不改题集**: gold 必须先于 prompt 冻结(§6.2 防线 1)。此刻改题集组成
= 在看过题集性质之后调整尺子,那正是本单元要防的形状。**记下来,不动它。**
