...
| # | 条款 | 触发后果 |
|---|---|---|
| 1 | 三遍**每一遍** fatal=0 且**旧 181 子集** exact ≥ **178** | 修法退回。**不许改 `EXACT_THRESHOLD` / 不许改 `score_run` 的 fatal 定义 / 不许从 gold 删题** |
...
收尾时一并更正。复算口径: `--junitxml` 的 `tests=1189 / failures=0 / errors=0 / skipped=0`
(非自洽写法 —— 不读 pytest 自己那行汇总,`-q` 下它没打出来)。
