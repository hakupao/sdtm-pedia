...
全展开 **131.3 KiB → 133.8 KiB (+2527 B ≈ +2.5 KB)**, 命令见 §5.3。最宽的 `C66742`
那行约 1.5K 字符, 远在 chunk 尺度内。

> 本行早先写作 "129.6 KB → 133.8 KB (+4.2 KB) 实测非估算" —— **错的, 且错得比不写更糟**:
> 129.6 KB 对不上 VI 任何一个历史版本 (真实旧值 131.3 KiB), 因为它把 Python 的**字符数**
> 与生成器打印的 **KiB** 混用, delta 报大 68%。规则 D 审查方与规则 A 抽检方各自独立抓到。
> 教训与本项目第 2 条硬规矩同源: **标着"实测"的数字必须真的来自那条附上的命令。**
...
真值 (字节级, 两条命令):
```bash
git show 6ed3d2b:knowledge_base/VARIABLE_INDEX.md | wc -c    # 134473
wc -c < knowledge_base/VARIABLE_INDEX.md                     # 137000
```
**134473 → 137000 B = +2527 B ≈ +2.5 KB**。补回的 226 条引用平均每条约 11 字节。
