...
| **D5** ⚠ | **Chat UI 主路径失去 fallback (本分支引入的回归)** —— 分支前 UI 永发 `default` 组, 该组有 `fallbacks=[{"default": ["default-fallback"]}]` 兜底; 分支后 UI **永发显式 id** (§5 裁定) 落 `opus-5`, 而四个新派生组**无 fallback 条目** ⇒ 主模型没变, **容灾网没了**。非理论风险: `sdtm-rag/DEPLOY_PLAN.md` 记着实测「Anthropic credits 耗尽 → DeepSeek 自动回退」**真的生效过** | ~~**本轮只记录不修**~~ → ✅ **已还 (2026-09-02)**。<br>*(当时不修的理由, 原文保留:* (a) 扩 fallback 表属 §1 明确划为非目标的 **U2**; (b) 更要紧 —— 加了会**激活 `model_id` 与 `model_used` 的分叉** (用户选 `gpt-sol` 却拿到 DeepSeek 的答案), 而 `model_used` **目前无任何取值断言** (见 D6), 等于在没有闸的地方引入新行为。*)*<br>⇒ **本轮正是照这个顺序还的: D6 先, D5 后。**<br>~~⛔ 在 U2 落地前, `opus-5` 失败时前端直接收到 `event: error`, 不再自动回退。~~ **已不成立**: 四个可选组均已补 fallback → `default-fallback` (表从 `selectable_models` 派生, `_fallback_map`), 且回退**必须被说出来** —— `done` 事件带 `models_used`/`fell_back`, 徽章写「模型: X → 实际 Y（已回退）· 验证状态未知」+ 琥珀色, 存档与 ⚑ 归因一并诚实。 详见 `docs/superpowers/specs/2026-09-02-model-fallback-honesty-design.md`; 分支 `fix/model-fallback-honesty` (`2bcd840..`), 全量 1956 → **2003 passed, 1 skipped**。 |
| **D6** | `model_used` 字段的**取值**在整个测试套件里无任何断言 (既有欠账, 非本分支引入) | ✅ **已还 (2026-09-02), 且确实先于 D5 落地**。补了发射端三向 (等于 Router 实际返回 / 跟着 Router 变 / 无人报告时发 `null` 而非写死 `"default"`) + 累积端两向 (后续 chunk 不报时**保值** / 只在 usage chunk 报时**收得到**)。⚠ 后两条是复审做变异才发现的: 三条发射端闸全绿时, 删掉累积端的 ` or model_used` **38 条无一变红** |
...
| **D11** | `/api/ask` 现在能用未验证模型答题 (I-3 修法把接受值 3→8 的必然结果), 但 `AskResponse` 没有 `model_id` / `verified` ⇒ **自证面缺一块** | 终审 M-7。⚠ **2026-09-02 加重, 未还**: 那一轮给四个可选组补 fallback 时, `/api/ask` **跟着获得了容灾** —— 实测 `model=opus-5` 由 **502 → 200 + 回退作答**。⇒ 本条的爆炸半径从 `default` **1 个组扩到 5 个**, 且**正好是用户能挑的那 4 个**。而 `model_used = getattr(response,"model",None) or body.model` 的**回显分支仍在** ⇒ provider 不报 `.model` 的回退会把 DeepSeek 的答案标成 `opus-5` (**C-1 同族**)。减轻事实 (终审核出): `ask_done` 日志已带 `model_used`, 且该回显分支实测是**死路**。行为变更已被闸钉住 (`test_ask_now_falls_back_instead_of_502`), 但**自证面仍缺**: `AskResponse` 至今没有 `fell_back`/`models_used`。见 2026-09-02 spec §7 **L5** |
