...
12. **三个搜索方向都要做** (本单元三次独立实证, 每次都抓到前一个方向漏掉的东西):
    - ①「**从断言出发**找能杀死它的变异」—— 上界 = 已有断言集合, **结构上发现不了无人守的
      代码行**。三个 task 的实现方都做了这个方向, 结论"断言全被证伪过"都为真。
    - ②「**从代码行出发**问这行改坏了谁会红」(Task 1/2 审查方) —— 抓到整块零覆盖代码:
      Task 3 实测删光 27 行装配块, 计划的 4 条断言**一条不红**; Task 1 的 9 条方向② 变异
      **全部存活**。
    - ③「**从断言的逻辑形状出发**找盲区」(Task 2 实现方反向自查) —— **对调型 (双点、
      差集不变) 变异是集合/差集类断言的系统性盲区**。同源比对省掉逐参数抄写, 代价是
      **丢失方向信息**: 只说两者"哪几个键不同", 不说"谁是谁"。实测 cards/docs 的 `top_k`
      对调后差集一字不变、全量全绿, 而那正是「加席不抢席」在接线层的反面。
      本单元两条 HIGH 里有**两条都是对调型** (引擎实参对调 / top_k 对调)。
