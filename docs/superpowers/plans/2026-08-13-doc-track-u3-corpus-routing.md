...
- **开工基线**: `1189 passed / 0 failed / 0 errors / 0 skipped`(Task 1 交付后为 **1207**);collections `sdtm_kb_v1 4329 / study_st01 959 / study_st01_docs 114`。对不上先查环境。
...
```
Expected: `1199 passed`(1189 + 10)。⚠ **两轮修复后终值为 1207**(+4 条 `main()` 测试,+4 条 avg 口径测试),下游预期数按 1207 起算
...
⚠ **给 controller 的纪律(不要出现在派给出题方的 prompt 里)**:派发时**不得**点名终审 held-out
是哪几道、也**不得**描述它们的题型 —— 泄漏任何一条,held-out 当场失效(spec §5.3 第 1 条)。
出题方**可以**通读 `test_set_docs_v1.yml` 全部 30 题(去重需要),要防的只是「知道哪几道特殊」。
**本段本身也不得随 brief 抽给出题方** —— 它一旦被抽走就成了泄漏源(本单元已因此栽过一次:
plan 初版把题型逐字写在这里,`task-brief` 原样抽进了出题方的需求文件)。
...
header = (
    ...
    "#   该规则保证恰好 12/12 且跨章对称; 「按章号奇偶」做不到 —— 24 题分布在 19 个章,\n"
    "#   奇数章 9 题 / 偶数章 15 题 (实测), 按章号奇偶切只能得到 9/15。\n"
...
- **规则文本写 pattern 不写 example**:规则必须描述**问题的对象**(本研究自己的决定 vs 标准的表示),
  **不得举任何具体临床概念作例子** —— 不许出现疾病名 / 解剖名 / 検査名 / 薬剤名 / 具体数値閾値 /
  域码 / 变量名 / 特定評価体系名 中的任何一类。(逐类平铺是有意的:只点其中几类等于给方向。)
  机械闸见 Step 4 的词表测试 —— 词表由 controller 预置在 gitignored 文件里,**你不需要也不许打开它**。
...
**4. 测试计数链**:1189(基线)→ **1207**(T1 +10 +8 两轮修复)→ 1221(T3 +14)→ 1228(T5 +7)→ Task 7 再 +2 = **1230**。
