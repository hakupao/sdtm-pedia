...
> **审题裁定 (2026-08-06)**: 独立审题判 REVISE (HIGH 3 / MEDIUM 6 / LOW 6)。其中最重要的一条是它
> **推翻了 lead 与出题人准备接受的妥协** —— 两道家族题 (`gold_max_matches` 20 / 30) 的"判别力≈0 作为
> 已知结构性限制"**不成立**: 二者都能改为单卡 gold 且零信息损失 (家族最后一枚卡的卡面本身就是"共 N 枠"
> 的直接证据, 比召回家族中随便一张更强)。修完后 **48 计分题里没有任何一题的 source_recall 是结构性白送的**。
> **该限制声明已收回, 不入档。**
>
> 另一条 lead 与出题人共同漏掉的缺陷: 其中一道家族题的 **gold 漏了第二问的答案卡** —— 3 条 expected_facts
> 里有 2 条只存在于那张非 gold 卡上, 即完全没答出第二问的系统照样满分。属 gold 不完整, 非判别力问题。
...
- [ ] **Step 5: 写 checkpoint** `sdtm-rag/evidence/checkpoints/study_golden_v2.md` — 只含统计与结构描述: 三版对照 (v0/v1.1/v2) / 配额与形态分布 / lint 闸结果 / 审题抓到的问题类型 / S2 开关两组数字 / 已知限制
  ...
  **必须写进已知限制的数字 (2026-08-06 已由审题人与 `audit_v2.py` 双方定准, 可直接引用)**:
  - **继承题一侧** (为保住与 v1.1 的历史可比性刻意不动, 属合理取舍, 但必须声明):
    | 形态 | 题数 |
    |---|---|
    | fact 命中 >100 卡 | 3 题 (最宽三条分别命中 **574 / 481 / 352** 卡) |
    | fact 命中 20–100 卡 | 3 题 |
    | **全部 fact 无判别力** (该题 fact_recall 结构上恒 = 1.0) | **1 题** |
    | **fact 已在题面出现** (回声即得分) | **7 题 / 7 条** |
  - **v2 新题一侧 (25 题)**: 上述四类**均为 0**。
  - 口径注: 审题人 R7 表原写 "6 题" 系其转录笔误, 已由本人更正为 7; `q23r` 归入"v2 新题"(本轮重写), 不影响上述继承题数字。
  - **不写这一段 = 用一把"1 题 fact 恒满分、7 题可靠回声拿分"的尺子对外报数字而不声明。**
