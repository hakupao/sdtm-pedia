...
| **gold 非唯一定位面比终审发现的更广** | 42 条 gold 中 **9 条**多卡匹配 (1 条 17 卡; 3 条 3 卡; 5 条 2 卡) | 需确定性 lint 闸 (Task 1) |
| **form 覆盖尚可但有死角** | 18/21 覆盖; 3 个未覆盖 form 合计 55 卡; 另有 1 个 17 卡 form 实质 0 题 (其唯一题的 gold 是家族前缀, 即判别力≈0 的那题) | 死角各补题 + 该题重写 (Task 2) |

注: q14 经实测确认是**真命中** (S2 注入 4 张卡全为同族, 无干扰卡, `source_hits` 记录 gold 本尊), 但其 gold 写法不具防伪能力 —— 属"当前正确但无保护", 正是 lint 要堵的形态。
