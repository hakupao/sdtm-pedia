...
**`anchors` 与之等长一一对应** —— 每个 gold 配自己那条锚串 (闸 B 方案 A, Task 6 前裁定)。

> ⚠ **本步初版写的是「`anchor` 选一条在这些 gold 里正好出现 N 次的串 —— 找不到就说明该题
> 其实单节可答, 改题或降型」。那句是错的, 已作废** (Task 6 开工前实测推翻):
> 规范化空白后, 恰好出现在 2 个 chunk 的 20 字 shingle 只覆盖 **1.4% 的 chunk 对**, 且有共享串
> 的多是父子/交叉引用节 —— 即**重复**的节。真正**互补**的跨节题本来就不该共享逐字长串。
> 按那句执行会把 8 道合法跨节题全降成单节, 批 2 的目的当场落空。
> 更根本的: 单条共享锚串证明的是"有句话同时出现在两块" = 重复的证据, 而跨节聚合题要的是
> **互补** —— 那不是严一点, 是**量错了东西**。逐 gold 锚串直接证明"答案确实分布在这几块"。
> 实测副产物: `22.1` 切点两侧**零可用锚串** (52 条"共享串"全是 -layout 的空白填充, 实质文本 0),
> 故旧口径下 §3 点名必做的 22.1 L6 探针**结构上写不出来**; 逐 gold 锚串把它救回来了。
...
> ⚠ **本步原写「至少 2 题瞄准 `8.2` 的 2→3 与 `22.1` 的 1→2 两个切点」, 该目标已于 2026-08-12
> 被全语料勘察证伪, 用户裁定改为「能加则加」** (勘察报告
> `.superpowers/sdd/2026-08-11-doc-track-u1-doc-question-set/task-6-recon-cutpoints.md`):
>
> 113 个切点全覆盖枚举 (无抽样) 判定 —— **可探针 1 · 边缘存疑 2 · 冗余 6 · PII 阻塞 27 ·
> 无劈开 76 · 空 chunk 1**。唯一可探针的就是 `8.2` 的 2→3, **即 `q41` 已占的那个**, 余料约 1 题
> (「脚注引用被斩断」)。另两个候选切点实测写不出: `22.1` 1→2 **未劈开任何答案**
> (只分开机构名簿本身); `8.2` 1→2 **一侧自足** (part02 的表把 part01 的散文结论复述了一遍)。
>
> 根因是一个被推翻的前提: 「切分器不总切在自然边界」不成立 —— 实测 **114 个 chunk 里 111 个的
> 正文首行就是它自己的节标题**, 例外的 3 个恰是 `__partNN` 续块。**真正的人为切点只有 3 个,
> 且已全部查过。** 故 L6 探针供给上限是 **2** (现存 `q41` + 本切点余料 1 题), 不是 ≥4。
>
> **不许为凑指标把「冗余」或「边缘」切点包装成探针** —— 假探针会让 U4 据以改切分器,
> 比没有证据更坏 (勘察方已明确拒绝交出 `#16` / `#88`)。

若在 `8.2` 的 2→3 切点上出到那道余料题, `note` 标 `L6_probe: true` **并加结构化键 `l6_probe: true`**。

> ⚠ **计数陷阱 (2026-08-12 修正)**: 原用「note 含 `L6_probe` 子串」计数, 而 `q40` 的**撤销说明**
> 自身含该子串, 被数成一道成立的探针。现 `q40` 的 note 已不含该字样, 真正成立的 `q41` 带结构化键。
> **后续统计一律以结构化键 `l6_probe` 为准。**

> ⛔ **取材禁区 (2026-08-12 实测)**: **`s10_3` 的 `表 10.3.4（1）` 与 `（2）` 所涵盖的主题不得出题**
> —— 这两个表的**表体在 PDF 文本层就已丢失** (原页 p079 / p080 实测: 表标题之后仅剩一条脚注与页码,
> 无任何表体行), **非切分器所致**。该主题在本语料**无 gold 可依**, 出题即造无解题。
> 这是 spec §8 所述 L9 盲点的第一个具名实例。
> (按红线只给**表编号 + chunk 文件名 + 页码**定位, 不复述表题文字。)
...
print('L6 探针:', sum(1 for q in qs if q.get('l6_probe')))"      # 结构化键, 不用 note 子串
```
> ⚠ 原命令用 `'L6_probe' in note` 子串计数, 已于 2026-08-12 证实会把**撤销说明**数成成立的探针
> (`q40` 的 note 记录撤销时自身含该子串)。**改用结构化键 `l6_probe`。**

Expected: `计分题: 30` · 配比 `single_section 12 / cross_section 8 / part_family 5 / table 5` ·
**~~`L6 探针 >= 4`~~ → `L6 探针 1 或 2`** (2026-08-12 用户裁定「能加则加」; 供给上限经
全语料 113 切点勘察确证为 **2**, 见 Step 2 的说明。**探针数不是 PASS 闸** —— spec §7 的三条
PASS 条件不含探针数) · 四闸 `0 条 finding`
