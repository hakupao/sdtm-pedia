...
- 2026-08-07 (VI 交叉引用完整性) — **VI §三 15 变量截断修完 ★★ 补回 226 条隐藏引用** (VI §三 上限 15 → 9/135 行被截隐藏 226 条; 域 spec 上限 5 → AE/LB 隐藏 10 条; 两个生成器上限全拆, KB 129.6→133.8 KB, 重灌 4303 chunks 与基线同数) — **⚠️ 本轮 v3 数字一动不动 (98.9286% 逐题 Δ0) 是预期而非没效果**: section 级判据只看 section 名、不看正文是否被截, 对本修复结构上失明, **引用本轮成果不得用 v3 数字**。能证明修好的是自带的两层尺子: 层① 数据不变量 `scripts/tests/test_kb_crossref_completeness.py` 3 红→5 绿 (135 行全覆盖, KB 层+chunk 层双打, 重灌前 C66742 chunk 缺 108 条); 层② context A/B 两题判别力均成立 (C66742→VSLOBXFL / C71620→URSTRESU, 截断答不出、完整答得出) + 独立 gold 集 fact 子串与 judge 双 100%; 生产端到端答案含旧语料结构上产不出的 VSLOBXFL。852→**857 passed**, reconcile_meta 8 项全 OK, freshness in sync — evidence: `sdtm-rag/evidence/checkpoints/vi_crossref_completeness.md` (含 5 条已知限制 + 两件必须点名的事: 顺带修好 PC/spec.md 自 06 以来陈旧的 Ch8 交叉引用; A/B 初版判据与答案表达形式不同语义, 把一次正确作答误判成失败)
