...
| **Phase 6.5 Gemini v7 draft (post R2 HIGH × 2 carry-over)** | **`ai_platforms/gemini_gems/dev/v7_draft/system_prompt_v7.md`** — v7 DRAFT (2026-04-23), 21,071 chars / 386 行, 基于 v6-post-A1 (18,716 chars); 2 patch: (1) CO-1c ARMCD null assignment rule (ARMCD/ARM null + ARMNRS C142179 Extensible 填全称, 禁 NOTASSGN / C66770 / 虚构 OBSERVATIONAL GROUP, R1+R2 Q13 (b) 系统性 gap 修) + (2) CO-4 §GF 正向清单双锚 (Core=Req/Exp 完整集 + 执行规则 3 条 + sanity 自检删重, R2 Q1 GFGENE regression HIGH 修); **v7 尚未应用到 Gem UI**, 待 reviewer 独审 + 用户 ack 后 apply |
| **Phase 6.5 ChatGPT v2.2 draft (post R1 MINOR carry-over)** | **`ai_platforms/chatgpt_gpt/dev/v2.2_draft/system_prompt_v2.2.md`** — v2.2 DRAFT (2026-04-23), 6,654 chars / 123 行, 基于 v2.1 live (5,681 chars); 1 bullet 加: "v3.4 新域变量名精确校验" (GFINHERT 7 字母 / 禁 GFINHERTG / CP 三 marker 8 字母 / BE+BS 短名), smoke v4 R1 Q1 ChatGPT 写成 GFINHERTG (extra G) PARTIAL MINOR 修; **v2.2 尚未应用到 GPT Builder UI**, live 仍 v2.1 (`chatgpt_gpt/current/system_prompt.md`) |
